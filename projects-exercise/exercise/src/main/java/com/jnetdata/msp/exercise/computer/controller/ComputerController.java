package com.jnetdata.msp.exercise.computer.controller;
 ...
 */
@Controller
@RequestMapping("/exercise/computer")
public class ComputerController extends BaseController<Long,Computer> {

    final private ComputerService computerService;
    ...
    @PostMapping
    @ResponseBody
    public JsonResult<EntityId<Long>> add(@Validated @RequestBody Computer entity) {
    ...
    public void syncAdd(Long columnid,Long fromid){
        Computer toCopy = new Computer();
        toCopy.setSingletempkate(0L);
        toCopy.setCopyfromid(fromid);

        Computer toYinyong = new Computer();
    ...
    @PostMapping("/batch")
    @ResponseBody
    public JsonResult addbatch( @RequestBody List<Computer> list) {
    ...
    @GetMapping("/{id}")
    @ResponseBody
    public JsonResult<Computer> getById(@PathVariable("id") Long id) {
    ...
    @GetMapping("/pub/{id}")
    @ResponseBody
    public JsonResult<Computer> pub(@PathVariable("id") Long id) {
        pubData(id);
        return renderSuccess();
    }
    ...
    @PostMapping("/pub")
    @ResponseBody
    public JsonResult<Computer> pubByIds(@RequestBody Long[] ids) {
        for (Long id : ids) {
            pubData(id);
        }
        return renderSuccess();
    ...
    @GetMapping("/repub/{id}")
    @ResponseBody
    public JsonResult<Computer> repub(@PathVariable("id") Long id) {
        repubData(id);
        return renderSuccess();
    }
    ...
    @PostMapping("/repub")
    @ResponseBody
    public JsonResult<Computer> repubByIds(@RequestBody Long[] ids) {
        for (Long id : ids) {
            repubData(id);
        }
        return renderSuccess();
    ...
    @GetMapping("/pass/{id}")
    @ResponseBody
    public JsonResult<Computer> pass(@PathVariable("id") Long id,String content) {
        passData(id,content);
        return renderSuccess();
    }
    ...
    @PostMapping("/pass")
    @ResponseBody
    public JsonResult<Computer> passByIds(@RequestBody Long[] ids,String content) {
        for (Long id : ids) {
            passData(id,content);
        }
        return renderSuccess();
    ...
    @ResponseBody
    public JsonResult<Computer> notpass(@PathVariable("id") Long id,String content) {
    ...
    @PostMapping("/notpass")
    @ResponseBody
    public JsonResult<Computer> notpassByIds(@RequestBody Long[] ids,String content) {
        for (Long id : ids) {
            notpassData(id,content);
        }
        return renderSuccess();
    ...
    public void passData(Long id,String content){
        Computer res = getService().getById(id);
        res.setStatus(10);
    ...
    public void pubData(Long id){
        ...
        if(!tocopycolumnId.isEmpty()){
            List<Computer> toSyncEdit = getService().list(new PropertyWrapper<>(Computer.class).eq("copyfromid", id).in("columnid", tocopycolumnId));
            if(!toSyncEdit.isEmpty()){
                for (Computer sync : toSyncEdit) {
    ...
    public JsonResult<Void> deleteBatchIds(@PathVariable("ids") String ids) {
        ...
        //删除已发布的
        List<Computer> entitys = getService().listByIds(Arrays.asList(idss));
        ...
        List<Computer> toDel = new ArrayList<>();
        for (String s : idss) {
            Computer entity = new Computer();
    ...
    public JsonResult<Void> batchdelIds(@PathVariable("ids") String ids,@PathVariable("columnid") Long columnid) {
        ...
        //删除同步的
        List<Computer> delSync = new ArrayList<>();
        for (Computer entity : entitys) {
    ...
    public void delSync(Long columnid,List<Computer> entitys) {
        for (Computer entity : entitys) {
    ...
    @PutMapping("/{id}")
    @ResponseBody
    public JsonResult<Void> updateById(@PathVariable("id") Long id, @RequestBody Computer entity) {
        List<Fenfa> fenfas = fenfaService.list(new PropertyWrapper<>(Fenfa.class)
        ...
        if(targetColumnIds.size() > 0){
            List<Computer> syncs = getService().list(new PropertyWrapper<>(Computer.class).in("columnid", targetColumnIds).eq("copyfromid", id));
            if(syncs.size() > 0) {
                for (Computer sync : syncs) {
                    Long syncId = sync.getId();
                    Long columnId = sync.getColumnid();
    ...
    @ResponseBody
    public JsonResult<Void> batchUpdate(@RequestBody List<Computer> list) {
    ...
    @ResponseBody
    public JsonResult<Void> doUpdateAllColumnById(@PathVariable("id") Long id, @RequestBody Computer entity) {
    ...
    @GetMapping("/{id}/join")
    @ResponseBody
    public JsonResult<Computer> getjoin(@PathVariable("id") Long id) {
        Computer entity = getService().getEntityAndJoinsById(id);
    ...
    @ResponseBody
    public JsonResult<Pager<Computer>> listjoin(@RequestBody BaseVo<Computer> vo) {
        JsonResult<Pager<Computer>> pagerJsonResult = doList(getService(), vo.getPager(), vo.getEntity());
        List<Computer> list = pagerJsonResult.getObj().getRecords();
    ...
    public JsonResult importData(String columnid, MultipartFile file) throws Exception {
        ...
        List<Fieldinfo> list = fieldinfoUtilService.list(new PropertyWrapper<>(Fieldinfo.class).eq("tablename", "jmeta_computer"));
        ...
        list1.forEach(item -> {
            Computer obj = MapUtil.map2Obj(item, Computer.class);
    ...
    @ResponseBody
    public JsonResult<Pager<Computer>> list(@ModelAttribute Computer entity,@RequestBody PageVo1 pager) {
    ...
    @ResponseBody
    public JsonResult<Pager<Computer>> list(@RequestBody BaseVo<Computer> vo) {
        if(vo.getEntity() == null) vo.setEntity(new Computer());
            vo.getEntity().setDocstatus(0);
            JsonResult<Pager<Computer>> result = doList(getService(), vo.getPager(), vo.getEntity());
            for (Computer record : result.getObj().getRecords()) {
    ...
    @ResponseBody
    public JsonResult<Pager<Computer>> delListing(@RequestBody BaseVo<Computer> vo) {
        if(vo.getEntity() == null) vo.setEntity(new Computer());
    ...
    @ResponseBody
    public JsonResult<Pager<Computer>> pubListing(@RequestBody BaseVo<Computer> vo) {
                if(vo.getEntity() == null) vo.setEntity(new Computer());
        List<Computer> list = getService().list(new PropertyWrapper<>(Computer.class).eq("status", "21").eq("docstatus", 0).select("docchannelid"));
    ...
    public void export(Computer entity, ExportVo vo, HttpServletResponse response) {
        List<Computer> datalist;
        if(vo.getArea() == 2){
            datalist = getService().list();
        }else{
            datalist = getService().list(createCondition(entity));
        }
        List<Fieldinfo> list = fieldinfoUtilService.list(new PropertyWrapper<>(Fieldinfo.class).eq("tablename", "jmeta_computer").eq("showList", 1));
        DownMetadataUtil.downMetadata(list,datalist,response,vo.getExt(),vo.getIncludeTitle(),Computer.class);
    }

    @ApiOperation(value = "导出", notes="根据vo指定条件进行导出")
    @PostMapping(value = "/export2")
    public void export2(@RequestBody ExportVo<Computer> vo, HttpServletResponse response) {
        List<Computer> datalist;
        if(vo.getArea() == 2){
            datalist = getService().list();
        }else{
    ...
    public void export3(ExportVo<Computer> vo, HttpServletResponse response) {
        List<Computer> datalist;
        if(vo.getArea() == 2){
            datalist = getService().list();
        }else{
        ...
        if(vo.getFields() != null){
            DownMetadataUtil.downMetadata(datalist,response,vo.getExt(),vo.getIncludeTitle(),Computer.class,vo.getFields());
        }else{
            List<Fieldinfo> list = fieldinfoUtilService.list(new PropertyWrapper<>(Fieldinfo.class).eq("tablename","jmeta_computer").eq("showList", 1));
            DownMetadataUtil.downMetadata(list,datalist,response,vo.getExt(),vo.getIncludeTitle(),Computer.class);
        }
    ...
    private ComputerService getService() {
