package com.jnetdata.msp.zsk.articleabstract.controller;
 ...
 */
@Controller
@RequestMapping("/zsk/articleabstract")
public class ArticleabstractController extends BaseController<Long,Articleabstract> {

    final private ArticleabstractService articleabstractService;
    ...
    @Autowired
    public ArticleabstractController(ArticleabstractService articleabstractService) {
        this.articleabstractService = articleabstractService;
    }
    ...
    @PostMapping
    @ResponseBody
    public JsonResult<EntityId<Long>> add(@Validated @RequestBody Articleabstract entity) {
        JsonResult<EntityId<Long>> result = doAdd(getService(), entity);
        return result;
    }

    public void syncAdd(Long columnid,Long fromid){
        Articleabstract toCopy = new Articleabstract();
        toCopy.setSingletempkate(0L);
        toCopy.setCopyfromid(fromid);

        Articleabstract toYinyong = new Articleabstract();
    ...
    @PostMapping("/batch")
    @ResponseBody
    public JsonResult addbatch( @RequestBody List<Articleabstract> list) {
    ...
    @GetMapping("/{id}")
    @ResponseBody
    public JsonResult<Articleabstract> getById(@PathVariable("id") Long id) {
    ...
    @GetMapping("/pub/{id}")
    @ResponseBody
    public JsonResult<Articleabstract> pub(@PathVariable("id") Long id) {
        pubData(id);
        return renderSuccess();
    }
    ...
    @PostMapping("/pub")
    @ResponseBody
    public JsonResult<Articleabstract> pubByIds(@RequestBody Long[] ids) {
        for (Long id : ids) {
            pubData(id);
        }
        return renderSuccess();
    ...
    @GetMapping("/repub/{id}")
    @ResponseBody
    public JsonResult<Articleabstract> repub(@PathVariable("id") Long id) {
        repubData(id);
        return renderSuccess();
    }
    ...
    @PostMapping("/repub")
    @ResponseBody
    public JsonResult<Articleabstract> repubByIds(@RequestBody Long[] ids) {
        for (Long id : ids) {
            repubData(id);
        }
        return renderSuccess();
    ...
    @GetMapping("/pass/{id}")
    @ResponseBody
    public JsonResult<Articleabstract> pass(@PathVariable("id") Long id,String content) {
        passData(id,content);
        return renderSuccess();
    }
    ...
    @PostMapping("/pass")
    @ResponseBody
    public JsonResult<Articleabstract> passByIds(@RequestBody Long[] ids,String content) {
        for (Long id : ids) {
            passData(id,content);
        }
        return renderSuccess();
    ...
    @ResponseBody
    public JsonResult<Articleabstract> notpass(@PathVariable("id") Long id,String content) {
    ...
    @PostMapping("/notpass")
    @ResponseBody
    public JsonResult<Articleabstract> notpassByIds(@RequestBody Long[] ids,String content) {
        for (Long id : ids) {
            notpassData(id,content);
        }
        return renderSuccess();
    ...
    public void passData(Long id,String content){
        Articleabstract res = getService().getById(id);
        res.setStatus(10);
    ...
    public void pubData(Long id){
        ...
        if(!tocopycolumnId.isEmpty()){
            List<Articleabstract> toSyncEdit = getService().list(new PropertyWrapper<>(Articleabstract.class).eq("copyfromid", id).in("columnid", tocopycolumnId));
            if(!toSyncEdit.isEmpty()){
                for (Articleabstract sync : toSyncEdit) {
    ...
    public JsonResult<Void> deleteById(@PathVariable("id") Long id) {
        ...
        Articleabstract entity = new Articleabstract();
        entity.setId(id);
    ...
    public JsonResult<Void> deleteBatchIds(@PathVariable("ids") @ApiParam("多个id用逗号隔开")String ids) {
        ...
        //删除已发布的
        List<Articleabstract> entitys = getService().listByIds(Arrays.asList(idss));
        ...
        List<Articleabstract> toDel = new ArrayList<>();
        for (String s : idss) {
            Articleabstract entity = new Articleabstract();
    ...
    public JsonResult<Void> batchdelIds(@PathVariable("ids") @ApiParam("多个id用逗号隔开")String ids,@PathVariable("columnid") Long columnid) {
        ...
        //删除同步的
        List<Articleabstract> delSync = new ArrayList<>();
        for (Articleabstract entity : entitys) {
    ...
    public void delSync(Long columnid,List<Articleabstract> entitys) {
        for (Articleabstract entity : entitys) {
    ...
    @PutMapping("/{id}")
    @ResponseBody
    public JsonResult<Void> updateById(@PathVariable("id") Long id, @RequestBody Articleabstract entity) {
        List<Fenfa> fenfas = fenfaService.list(new PropertyWrapper<>(Fenfa.class)
        ...
        if(targetColumnIds.size() > 0){
            List<Articleabstract> syncs = getService().list(new PropertyWrapper<>(Articleabstract.class).in("columnid", targetColumnIds).eq("copyfromid", id));
            if(syncs.size() > 0) {
                for (Articleabstract sync : syncs) {
                    Long syncId = sync.getId();
                    Long columnId = sync.getColumnid();
    ...
    @ResponseBody
    public JsonResult<Void> batchUpdate(@RequestBody List<Articleabstract> list) {
    ...
    public JsonResult<Void> updateBatchById(@RequestBody Long[] ids, Integer docstatus) {
        List<Articleabstract> list = new ArrayList<>();
        for (Long id : ids) {
            Articleabstract entity = new Articleabstract();
            entity.setId(id);
    ...
    @ResponseBody
    public JsonResult<Void> doUpdateAllColumnById(@PathVariable("id") Long id, @RequestBody Articleabstract entity) {
    ...
    @GetMapping("/{id}/join")
    @ResponseBody
    public JsonResult<Articleabstract> getjoin(@PathVariable("id") Long id) {
        Articleabstract entity = getService().getEntityAndJoinsById(id);
    ...
    @ResponseBody
    public JsonResult<Pager<Articleabstract>> listjoin(@RequestBody BaseVo<Articleabstract> vo) {
        JsonResult<Pager<Articleabstract>> pagerJsonResult = doList(getService(), vo.getPager(), vo.getEntity());
        List<Articleabstract> list = pagerJsonResult.getObj().getRecords();
    ...
    public JsonResult importData(String columnid, MultipartFile file) throws Exception {
        ...
        List<Fieldinfo> list = fieldinfoUtilService.list(new PropertyWrapper<>(Fieldinfo.class).eq("tablename", "JMETA_ARTICLEABSTRACT"));
        ...
        List<Articleabstract> list2 = new ArrayList<>();
        list1.forEach(item -> {
            Articleabstract obj = MapUtil.map2Obj(item, Articleabstract.class);
    ...
    @ResponseBody
    public JsonResult<Pager<Articleabstract>> list(@ModelAttribute Articleabstract entity,@RequestBody PageVo1 pager) {
    ...
    @ResponseBody
    public JsonResult<Pager<Articleabstract>> list(@RequestBody BaseVo<Articleabstract> vo) {
        if(vo.getEntity() == null) vo.setEntity(new Articleabstract());
            vo.getEntity().setDocstatus(0);
            JsonResult<Pager<Articleabstract>> result = doList(getService(), vo.getPager(), vo.getEntity());
            for (Articleabstract record : result.getObj().getRecords()) {
    ...
    @ResponseBody
    public JsonResult<Pager<Articleabstract>> delListing(@RequestBody BaseVo<Articleabstract> vo) {
        if(vo.getEntity() == null) vo.setEntity(new Articleabstract());
    ...
    @ResponseBody
    public JsonResult<Pager<Articleabstract>> pubListing(@RequestBody BaseVo<Articleabstract> vo) {
                if(vo.getEntity() == null) vo.setEntity(new Articleabstract());
        List<Articleabstract> list = getService().list(new PropertyWrapper<>(Articleabstract.class).eq("status", "21").eq("docstatus", 0).select("docchannelid"));
    ...
    public void export(Articleabstract entity, ExportVo vo, HttpServletResponse response) {
        List<Articleabstract> datalist;
        if(vo.getArea() == 2){
            datalist = getService().list();
        }else{
            datalist = getService().list(createCondition(entity));
        }
        List<Fieldinfo> list = fieldinfoUtilService.list(new PropertyWrapper<>(Fieldinfo.class).eq("tablename", "JMETA_ARTICLEABSTRACT").eq("showList", 1));
        DownMetadataUtil.downMetadata(list,datalist,response,vo.getExt(),vo.getIncludeTitle(),Articleabstract.class);
    }

    @ApiOperation(value = "根据json参数导出全部数据", notes="根据json参数导出全部数据")
    @PostMapping(value = "/export2")
    public void export2(@RequestBody ExportVo<Articleabstract> vo, HttpServletResponse response) {
        List<Articleabstract> datalist;
        if(vo.getArea() == 2){
            datalist = getService().list();
        }else{
    ...
    public void export3(ExportVo<Articleabstract> vo, HttpServletResponse response) {
        List<Articleabstract> datalist;
        if(vo.getArea() == 2){
            datalist = getService().list();
        }else{
        ...
        if(vo.getFields() != null){
            DownMetadataUtil.downMetadata(datalist,response,vo.getExt(),vo.getIncludeTitle(),Articleabstract.class,vo.getFields());
        }else{
            List<Fieldinfo> list = fieldinfoUtilService.list(new PropertyWrapper<>(Fieldinfo.class).eq("tablename","JMETA_ARTICLEABSTRACT").eq("showList", 1));
            DownMetadataUtil.downMetadata(list,datalist,response,vo.getExt(),vo.getIncludeTitle(),Articleabstract.class);
        }
    ...
    private ArticleabstractService getService() {
        return this.articleabstractService;
    }
