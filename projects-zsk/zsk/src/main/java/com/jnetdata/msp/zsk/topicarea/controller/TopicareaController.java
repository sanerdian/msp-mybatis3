package com.jnetdata.msp.zsk.topicarea.controller;
 ...
 */
@Controller
@RequestMapping("/zsk/topicarea")
public class TopicareaController extends BaseController<Long,Topicarea> {

    final private TopicareaService topicareaService;
    ...
    @Autowired
    public TopicareaController(TopicareaService topicareaService) {
    ...
    public JsonResult<EntityId<Long>> add(@Validated @RequestBody Topicarea entity) {
        entity.setDocstatus(0);
    ...
    public void syncAdd(Long columnid,Long fromid){
        Topicarea toCopy = new Topicarea();
        toCopy.setSingletempkate(0L);
        toCopy.setCopyfromid(fromid);

        Topicarea toYinyong = new Topicarea();
    ...
    @PostMapping("/batch")
    @ResponseBody
    public JsonResult addbatch( @RequestBody List<Topicarea> list) {
    ...
    @GetMapping("/{id}")
    @ResponseBody
    public JsonResult<Topicarea> getById(@PathVariable("id") Long id) {
    ...
    @GetMapping("/pub/{id}")
    @ResponseBody
    public JsonResult<Topicarea> pub(@PathVariable("id") Long id) {
        pubData(id);
        return renderSuccess();
    }
    ...
    @PostMapping("/pub")
    @ResponseBody
    public JsonResult<Topicarea> pubByIds(@RequestBody Long[] ids) {
        for (Long id : ids) {
            pubData(id);
        }
        return renderSuccess();
    ...
    @GetMapping("/repub/{id}")
    @ResponseBody
    public JsonResult<Topicarea> repub(@PathVariable("id") Long id) {
        repubData(id);
        return renderSuccess();
    }
    ...
    @PostMapping("/repub")
    @ResponseBody
    public JsonResult<Topicarea> repubByIds(@RequestBody Long[] ids) {
        for (Long id : ids) {
            repubData(id);
        }
        return renderSuccess();
    ...
    @GetMapping("/pass/{id}")
    @ResponseBody
    public JsonResult<Topicarea> pass(@PathVariable("id") Long id,String content) {
        passData(id,content);
        return renderSuccess();
    }
    ...
    @PostMapping("/pass")
    @ResponseBody
    public JsonResult<Topicarea> passByIds(@RequestBody Long[] ids,String content) {
        for (Long id : ids) {
            passData(id,content);
        }
        return renderSuccess();
    ...
    @ResponseBody
    public JsonResult<Topicarea> notpass(@PathVariable("id") Long id,String content) {
    ...
    @PostMapping("/notpass")
    @ResponseBody
    public JsonResult<Topicarea> notpassByIds(@RequestBody Long[] ids,String content) {
        for (Long id : ids) {
            notpassData(id,content);
        }
        return renderSuccess();
    ...
    public void passData(Long id,String content){
        Topicarea res = getService().getById(id);
        res.setStatus(10);
    ...
    public void pubData(Long id){
        ...
        if(!tocopycolumnId.isEmpty()){
            List<Topicarea> toSyncEdit = getService().list(new PropertyWrapper<>(Topicarea.class).eq("copyfromid", id).in("columnid", tocopycolumnId));
            if(!toSyncEdit.isEmpty()){
                for (Topicarea sync : toSyncEdit) {
    ...
    public JsonResult<Void> deleteById(@PathVariable("id") Long id) {
        ...
        Topicarea entity = new Topicarea();
        entity.setId(id);
    ...
    public JsonResult<Void> deleteBatchIds(@PathVariable("ids") @ApiParam("多个id用逗号隔开")String ids) {
        ...
        //删除已发布的
        List<Topicarea> entitys = getService().listByIds(Arrays.asList(idss));
        ...
        List<Topicarea> toDel = new ArrayList<>();
        for (String s : idss) {
            Topicarea entity = new Topicarea();
    ...
    public JsonResult<Void> batchdelIds(@PathVariable("ids") @ApiParam("多个id用逗号隔开")String ids,@PathVariable("columnid") Long columnid) {
        ...
        //删除同步的
        List<Topicarea> delSync = new ArrayList<>();
        for (Topicarea entity : entitys) {
    ...
    public void delSync(Long columnid,List<Topicarea> entitys) {
        for (Topicarea entity : entitys) {
    ...
    @PutMapping("/{id}")
    @ResponseBody
    public JsonResult<Void> updateById(@PathVariable("id") Long id, @RequestBody Topicarea entity) {
        List<Fenfa> fenfas = fenfaService.list(new PropertyWrapper<>(Fenfa.class)
        ...
        if(targetColumnIds.size() > 0){
            List<Topicarea> syncs = getService().list(new PropertyWrapper<>(Topicarea.class).in("columnid", targetColumnIds).eq("copyfromid", id));
            if(syncs.size() > 0) {
                for (Topicarea sync : syncs) {
                    Long syncId = sync.getId();
                    Long columnId = sync.getColumnid();
    ...
    @ResponseBody
    public JsonResult<Void> batchUpdate(@RequestBody List<Topicarea> list) {
    ...
    public JsonResult<Void> updateBatchById(@RequestBody Long[] ids, Integer docstatus) {
        List<Topicarea> list = new ArrayList<>();
        for (Long id : ids) {
            Topicarea entity = new Topicarea();
            entity.setId(id);
    ...
    @ResponseBody
    public JsonResult<Void> doUpdateAllColumnById(@PathVariable("id") Long id, @RequestBody Topicarea entity) {
    ...
    @GetMapping("/{id}/join")
    @ResponseBody
    public JsonResult<Topicarea> getjoin(@PathVariable("id") Long id) {
        Topicarea entity = getService().getEntityAndJoinsById(id);
    ...
    @ResponseBody
    public JsonResult<Pager<Topicarea>> listjoin(@RequestBody BaseVo<Topicarea> vo) {
        JsonResult<Pager<Topicarea>> pagerJsonResult = doList(getService(), vo.getPager(), vo.getEntity());
        List<Topicarea> list = pagerJsonResult.getObj().getRecords();
    ...
    public JsonResult importData(String columnid, MultipartFile file) throws Exception {
        ...
        List<Fieldinfo> list = fieldinfoUtilService.list(new PropertyWrapper<>(Fieldinfo.class).eq("tablename", "JMETA_TOPICAREA"));
        ...
        List<Topicarea> list2 = new ArrayList<>();
        list1.forEach(item -> {
            Topicarea obj = MapUtil.map2Obj(item, Topicarea.class);
    ...
    @ResponseBody
    public JsonResult<Pager<Topicarea>> list(@ModelAttribute Topicarea entity,@RequestBody PageVo1 pager) {
    ...
    @ResponseBody
    public JsonResult<Pager<Topicarea>> list(@RequestBody BaseVo<Topicarea> vo) {
        if(vo.getEntity() == null) vo.setEntity(new Topicarea());
            vo.getEntity().setDocstatus(0);
            JsonResult<Pager<Topicarea>> result = doList(getService(), vo.getPager(), vo.getEntity());
            for (Topicarea record : result.getObj().getRecords()) {
    ...
    @ResponseBody
    public JsonResult<Pager<Topicarea>> delListing(@RequestBody BaseVo<Topicarea> vo) {
        if(vo.getEntity() == null) vo.setEntity(new Topicarea());
    ...
    @ResponseBody
    public JsonResult<Pager<Topicarea>> pubListing(@RequestBody BaseVo<Topicarea> vo) {
                if(vo.getEntity() == null) vo.setEntity(new Topicarea());
        List<Topicarea> list = getService().list(new PropertyWrapper<>(Topicarea.class).eq("status", "21").eq("docstatus", 0).select("docchannelid"));
    ...
    public void export(Topicarea entity, ExportVo vo, HttpServletResponse response) {
        List<Topicarea> datalist;
        if(vo.getArea() == 2){
            datalist = getService().list();
        }else{
            datalist = getService().list(createCondition(entity));
        }
        List<Fieldinfo> list = fieldinfoUtilService.list(new PropertyWrapper<>(Fieldinfo.class).eq("tablename", "JMETA_TOPICAREA").eq("showList", 1));
        DownMetadataUtil.downMetadata(list,datalist,response,vo.getExt(),vo.getIncludeTitle(),Topicarea.class);
    }

    @ApiOperation(value = "根据json参数导出全部数据", notes="根据vo指定条件进行导出")
    @PostMapping(value = "/export2")
    public void export2(@RequestBody ExportVo<Topicarea> vo, HttpServletResponse response) {
        List<Topicarea> datalist;
        if(vo.getArea() == 2){
            datalist = getService().list();
        }else{
    ...
    public void export3(ExportVo<Topicarea> vo, HttpServletResponse response) {
        List<Topicarea> datalist;
        if(vo.getArea() == 2){
            datalist = getService().list();
        }else{
        ...
        if(vo.getFields() != null){
            DownMetadataUtil.downMetadata(datalist,response,vo.getExt(),vo.getIncludeTitle(),Topicarea.class,vo.getFields());
        }else{
            List<Fieldinfo> list = fieldinfoUtilService.list(new PropertyWrapper<>(Fieldinfo.class).eq("tablename","JMETA_TOPICAREA").eq("showList", 1));
            DownMetadataUtil.downMetadata(list,datalist,response,vo.getExt(),vo.getIncludeTitle(),Topicarea.class);
        }
    ...
    private TopicareaService getService() {
        return this.topicareaService;
    }
