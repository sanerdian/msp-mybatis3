package com.jnetdata.msp.zsk.people.controller;
 ...
 */
@Controller
@RequestMapping("/zsk/people")
public class PeopleController extends BaseController<Long,People> {
    ...
    public JsonResult<EntityId<Long>> add(@Validated @RequestBody People entity) {
    ...
    @PostMapping("/batch")
    @ResponseBody
    public JsonResult addbatch( @RequestBody List<People> list) {
    ...
    @GetMapping("/{id}")
    @ResponseBody
    public JsonResult<People> getById(@PathVariable("id") Long id) {
    ...
    @GetMapping("/pub/{id}")
    @ResponseBody
    public JsonResult<People> pub(@PathVariable("id") Long id) {
        pubData(id);
        return renderSuccess();
    }
    ...
    @PostMapping("/pub")
    @ResponseBody
    public JsonResult<People> pubByIds(@RequestBody Long[] ids) {
        for (Long id : ids) {
            pubData(id);
        }
        return renderSuccess();
    ...
    @GetMapping("/repub/{id}")
    @ResponseBody
    public JsonResult<People> repub(@PathVariable("id") Long id) {
        repubData(id);
        return renderSuccess();
    }
    ...
    @PostMapping("/repub")
    @ResponseBody
    public JsonResult<People> repubByIds(@RequestBody Long[] ids) {
        for (Long id : ids) {
            repubData(id);
        }
        return renderSuccess();
    ...
    @GetMapping("/pass/{id}")
    @ResponseBody
    public JsonResult<People> pass(@PathVariable("id") Long id,String content) {
        passData(id,content);
        return renderSuccess();
    }
    ...
    @PostMapping("/pass")
    @ResponseBody
    public JsonResult<People> passByIds(@RequestBody Long[] ids,String content) {
        for (Long id : ids) {
            passData(id,content);
        }
        return renderSuccess();
    ...
    @ResponseBody
    public JsonResult<People> notpass(@PathVariable("id") Long id,String content) {
    ...
    @PostMapping("/notpass")
    @ResponseBody
    public JsonResult<People> notpassByIds(@RequestBody Long[] ids,String content) {
        for (Long id : ids) {
            notpassData(id,content);
        }
        return renderSuccess();
    ...
    public void passData(Long id,String content){
        People res = getService().getById(id);
        res.setStatus(10);
    ...
    public void pubData(Long id){
        ...
        if(!tocopycolumnId.isEmpty()){
            List<People> toSyncEdit = getService().list(new PropertyWrapper<>(People.class).eq("copyfromid", id).in("columnid", tocopycolumnId));
            if(!toSyncEdit.isEmpty()){
                for (People sync : toSyncEdit) {
    ...
    public JsonResult<Void> deleteBatchIds(@PathVariable("ids") @ApiParam("多个id用逗号隔开")String ids) {
        ...
        //删除已发布的
        List<People> entitys = getService().listByIds(Arrays.asList(idss));
        ...
        List<People> toDel = new ArrayList<>();
        for (String s : idss) {
            People entity = new People();
    ...
    public JsonResult<Void> batchdelIds(@PathVariable("ids") @ApiParam("多个id用逗号隔开")String ids,@PathVariable("columnid") Long columnid) {
        ...
        //删除同步的
        List<People> delSync = new ArrayList<>();
        for (People entity : entitys) {
    ...
    public void delSync(Long columnid,List<People> entitys) {
        for (People entity : entitys) {
    ...
    @PutMapping("/{id}")
    @ResponseBody
    public JsonResult<Void> updateById(@PathVariable("id") Long id, @RequestBody People entity) {
        List<Fenfa> fenfas = fenfaService.list(new PropertyWrapper<>(Fenfa.class)
        ...
        if(targetColumnIds.size() > 0){
            List<People> syncs = getService().list(new PropertyWrapper<>(People.class).in("columnid", targetColumnIds).eq("copyfromid", id));
            if(syncs.size() > 0) {
                for (People sync : syncs) {
                    Long syncId = sync.getId();
                    Long columnId = sync.getColumnid();
    ...
    @ResponseBody
    public JsonResult<Void> batchUpdate(@RequestBody List<People> list) {
    ...
    @ResponseBody
    public JsonResult<Void> doUpdateAllColumnById(@PathVariable("id") Long id, @RequestBody People entity) {
    ...
    @GetMapping("/{id}/join")
    @ResponseBody
    public JsonResult<People> getjoin(@PathVariable("id") Long id) {
        People entity = getService().getEntityAndJoinsById(id);
    ...
    @ResponseBody
    public JsonResult<Pager<People>> listjoin(@RequestBody BaseVo<People> vo) {
        JsonResult<Pager<People>> pagerJsonResult = doList(getService(), vo.getPager(), vo.getEntity());
        List<People> list = pagerJsonResult.getObj().getRecords();
    ...
    public JsonResult importData(String columnid, MultipartFile file) throws Exception {
        ...
        List<Fieldinfo> list = fieldinfoUtilService.list(new PropertyWrapper<>(Fieldinfo.class).eq("tablename", "JMETA_PEOPLE"));
        ...
        list1.forEach(item -> {
            People obj = MapUtil.map2Obj(item, People.class);
    ...
    @ResponseBody
    public JsonResult<Pager<People>> list(@ModelAttribute People entity,@RequestBody PageVo1 pager) {
    ...
    @ResponseBody
    public JsonResult<Pager<People>> list(@RequestBody BaseVo<People> vo) {
        if(vo.getEntity() == null) vo.setEntity(new People());
            vo.getEntity().setDocstatus(0);
            JsonResult<Pager<People>> result = doList(getService(), vo.getPager(), vo.getEntity());
            for (People record : result.getObj().getRecords()) {
    ...
    @ResponseBody
    public JsonResult<Pager<People>> delListing(@RequestBody BaseVo<People> vo) {
        if(vo.getEntity() == null) vo.setEntity(new People());
    ...
    @ResponseBody
    public JsonResult<Pager<People>> pubListing(@RequestBody BaseVo<People> vo) {
                if(vo.getEntity() == null) vo.setEntity(new People());
        List<People> list = getService().list(new PropertyWrapper<>(People.class).eq("status", "21").eq("docstatus", 0).select("docchannelid"));
    ...
    public void export(People entity, ExportVo vo, HttpServletResponse response) {
        ...
        List<Fieldinfo> list = fieldinfoUtilService.list(new PropertyWrapper<>(Fieldinfo.class).eq("tablename", "JMETA_PEOPLE").eq("showList", 1));
        DownMetadataUtil.downMetadata(list,datalist,response,vo.getExt(),vo.getIncludeTitle(),People.class);
    }

    @ApiOperation(value = "根据json参数导出全部数据", notes="根据vo指定条件进行导出")
    @PostMapping(value = "/export2")
    public void export2(@RequestBody ExportVo<People> vo, HttpServletResponse response) {
        List<People> datalist;
        if(vo.getArea() == 2){
            datalist = getService().list();
        }else{
    ...
    public void export3(ExportVo<People> vo, HttpServletResponse response) {
        List<People> datalist;
        if(vo.getArea() == 2){
            datalist = getService().list();
        }else{
        ...
        if(vo.getFields() != null){
            DownMetadataUtil.downMetadata(datalist,response,vo.getExt(),vo.getIncludeTitle(),People.class,vo.getFields());
        }else{
            List<Fieldinfo> list = fieldinfoUtilService.list(new PropertyWrapper<>(Fieldinfo.class).eq("tablename","JMETA_PEOPLE").eq("showList", 1));
            DownMetadataUtil.downMetadata(list,datalist,response,vo.getExt(),vo.getIncludeTitle(),People.class);
        }
    ...
    private PeopleService getService() {
