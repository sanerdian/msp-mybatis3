package com.jnetdata.msp.zsk.unit.controller;
 ...
 */
@Controller
@RequestMapping("/zsk/unit")
public class UnitController extends BaseController<Long,Unit> {
    ...
    public JsonResult<EntityId<Long>> add(@Validated @RequestBody Unit entity) {
    ...
    @PostMapping("/batch")
    @ResponseBody
    public JsonResult addbatch( @RequestBody List<Unit> list) {
    ...
    @GetMapping("/{id}")
    @ResponseBody
    public JsonResult<Unit> getById(@PathVariable("id") Long id) {
    ...
    @GetMapping("/pub/{id}")
    @ResponseBody
    public JsonResult<Unit> pub(@PathVariable("id") Long id) {
        pubData(id);
        return renderSuccess();
    }
    ...
    @PostMapping("/pub")
    @ResponseBody
    public JsonResult<Unit> pubByIds(@RequestBody Long[] ids) {
        for (Long id : ids) {
            pubData(id);
        }
        return renderSuccess();
    ...
    @GetMapping("/repub/{id}")
    @ResponseBody
    public JsonResult<Unit> repub(@PathVariable("id") Long id) {
        repubData(id);
        return renderSuccess();
    }
    ...
    @PostMapping("/repub")
    @ResponseBody
    public JsonResult<Unit> repubByIds(@RequestBody Long[] ids) {
        for (Long id : ids) {
            repubData(id);
        }
        return renderSuccess();
    ...
    @GetMapping("/pass/{id}")
    @ResponseBody
    public JsonResult<Unit> pass(@PathVariable("id") Long id,String content) {
        passData(id,content);
        return renderSuccess();
    }
    ...
    @PostMapping("/pass")
    @ResponseBody
    public JsonResult<Unit> passByIds(@RequestBody Long[] ids,String content) {
        for (Long id : ids) {
            passData(id,content);
        }
        return renderSuccess();
    ...
    @ResponseBody
    public JsonResult<Unit> notpass(@PathVariable("id") Long id,String content) {
    ...
    @PostMapping("/notpass")
    @ResponseBody
    public JsonResult<Unit> notpassByIds(@RequestBody Long[] ids,String content) {
        for (Long id : ids) {
            notpassData(id,content);
        }
        return renderSuccess();
    ...
    public void passData(Long id,String content){
        Unit res = getService().getById(id);
        res.setStatus(10);
    ...
    public void pubData(Long id){
        ...
        if(!tocopycolumnId.isEmpty()){
            List<Unit> toSyncEdit = getService().list(new PropertyWrapper<>(Unit.class).eq("copyfromid", id).in("columnid", tocopycolumnId));
            if(!toSyncEdit.isEmpty()){
                for (Unit sync : toSyncEdit) {
    ...
    public JsonResult<Void> deleteBatchIds(@PathVariable("ids") String ids) {
        ...
        //删除已发布的
        List<Unit> entitys = getService().listByIds(Arrays.asList(idss));
    ...
    public JsonResult<Void> batchdelIds(@PathVariable("ids") String ids,@PathVariable("columnid") Long columnid) {
        ...
        //删除同步的
        List<Unit> delSync = new ArrayList<>();
        for (Unit entity : entitys) {
    ...
    public void delSync(Long columnid,List<Unit> entitys) {
        for (Unit entity : entitys) {
    ...
    @PutMapping("/{id}")
    @ResponseBody
    public JsonResult<Void> updateById(@PathVariable("id") Long id, @RequestBody Unit entity) {
        List<Fenfa> fenfas = fenfaService.list(new PropertyWrapper<>(Fenfa.class)
        ...
        if(targetColumnIds.size() > 0){
            List<Unit> syncs = getService().list(new PropertyWrapper<>(Unit.class).in("columnid", targetColumnIds).eq("copyfromid", id));
            if(syncs.size() > 0) {
                for (Unit sync : syncs) {
                    Long syncId = sync.getId();
                    Long columnId = sync.getColumnid();
    ...
    @ResponseBody
    public JsonResult<Void> batchUpdate(@RequestBody List<Unit> list) {
    ...
    @ResponseBody
    public JsonResult<Void> doUpdateAllColumnById(@PathVariable("id") Long id, @RequestBody Unit entity) {
    ...
    @GetMapping("/{id}/join")
    @ResponseBody
    public JsonResult<Unit> getjoin(@PathVariable("id") Long id) {
        Unit entity = getService().getEntityAndJoinsById(id);
    ...
    @ResponseBody
    public JsonResult<Pager<Unit>> listjoin(@RequestBody BaseVo<Unit> vo) {
        JsonResult<Pager<Unit>> pagerJsonResult = doList(getService(), vo.getPager(), vo.getEntity());
        List<Unit> list = pagerJsonResult.getObj().getRecords();
    ...
    public JsonResult importData(String columnid, MultipartFile file) throws Exception {
        ...
        List<Fieldinfo> list = fieldinfoUtilService.list(new PropertyWrapper<>(Fieldinfo.class).eq("tablename", "JMETA_UNIT"));
        ...
        list1.forEach(item -> {
            Unit obj = MapUtil.map2Obj(item, Unit.class);
    ...
    @ResponseBody
    public JsonResult<Pager<Unit>> list(@ModelAttribute Unit entity,@RequestBody PageVo1 pager) {
    ...
    @ResponseBody
    public JsonResult<Pager<Unit>> list(@RequestBody BaseVo<Unit> vo) {
        if(vo.getEntity() == null) vo.setEntity(new Unit());
            vo.getEntity().setDocstatus(0);
            JsonResult<Pager<Unit>> result = doList(getService(), vo.getPager(), vo.getEntity());
            for (Unit record : result.getObj().getRecords()) {
    ...
    @ResponseBody
    public JsonResult<Pager<Unit>> delListing(@RequestBody BaseVo<Unit> vo) {
        if(vo.getEntity() == null) vo.setEntity(new Unit());
    ...
    @ResponseBody
    public JsonResult<Pager<Unit>> pubListing(@RequestBody BaseVo<Unit> vo) {
                if(vo.getEntity() == null) vo.setEntity(new Unit());
        List<Unit> list = getService().list(new PropertyWrapper<>(Unit.class).eq("status", "21").eq("docstatus", 0).select("docchannelid"));
    ...
    public void export(Unit entity, ExportVo vo, HttpServletResponse response) {
        List<Unit> datalist;
        if(vo.getArea() == 2){
        ...
        List<Fieldinfo> list = fieldinfoUtilService.list(new PropertyWrapper<>(Fieldinfo.class).eq("tablename", "JMETA_UNIT").eq("showList", 1));
        DownMetadataUtil.downMetadata(list,datalist,response,vo.getExt(),vo.getIncludeTitle(),Unit.class);
    }

    @ApiOperation(value = "根据json参数导出全部数据", notes="根据json参数导出全部数据")
    @PostMapping(value = "/export2")
    public void export2(@RequestBody ExportVo<Unit> vo, HttpServletResponse response) {
        List<Unit> datalist;
        if(vo.getArea() == 2){
            datalist = getService().list();
        }else{
    ...
    public void export3(ExportVo<Unit> vo, HttpServletResponse response) {
        List<Unit> datalist;
        if(vo.getArea() == 2){
            datalist = getService().list();
        }else{
        ...
        if(vo.getFields() != null){
            DownMetadataUtil.downMetadata(datalist,response,vo.getExt(),vo.getIncludeTitle(),Unit.class,vo.getFields());
        }else{
            List<Fieldinfo> list = fieldinfoUtilService.list(new PropertyWrapper<>(Fieldinfo.class).eq("tablename","JMETA_UNIT").eq("showList", 1));
            DownMetadataUtil.downMetadata(list,datalist,response,vo.getExt(),vo.getIncludeTitle(),Unit.class);
        }
    ...
    private UnitService getService() {
