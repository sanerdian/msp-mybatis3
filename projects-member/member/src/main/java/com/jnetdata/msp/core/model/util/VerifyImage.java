...
public class VerifyImage extends HttpServlet {
	 ...
	 * 
	 */
	private static Logger logger = LoggerFactory.getLogger(VerifyImage.class);
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		logger.info("验证码:"+test);


		req.getSession().setAttribute("rand", test);// 将字符串写入session

		//将验证码放入session缓存
		String verifyType = req.getParameter("verifyType").toString();
		Session session= SecurityUtils.getSubject().getSession();
		session.setAttribute(verifyType, test);
		...
		bi=null;
		sos.flush();  
		//sos.close();    //原先这有sos.close(); 我把这个注释掉错误就不报了
	}
