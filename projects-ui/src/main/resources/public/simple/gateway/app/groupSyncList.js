...
let param4GroupSync={"appId":appId,"objectType":"group"};
init();
function init() {
    doList4GroupSync(1);
    form.on("submit(LAY-groupSync-back-search)",function (data) {
        Object.assign(param4GroupSync,data.field)
        doList4GroupSync(1);
        return false;
    });
    //点击按钮操作
    $('.layui-btn').click(function(){
        ...
        if(type=='reset'){
            let ids=checkChecked("tableGroupSync");
            doReset4GroupSync(ids);
        }else if(type=='deletes'){
            let ids=checkChecked("tableGroupSync");
            doDeleteByIds4GroupSync(ids);
        }
...
//条件列表搜索
function doList4GroupSync(current) {
    let columns=[
        {type:'checkbox'}
    ...
    doListTable(service_prefix.gateway, "/taskResult/list","post",param4GroupSync,current,null
        ,"tableGroupSync",columns,"pageGroupSync");
}//根据ids批量删除
function doDeleteByIds4GroupSync(ids) {
...
function doReset4GroupSync(ids) {
