 ...
 */
 function WidgetFoot() {
    console.log('Widget foot被调用');
    this.create.apply(this, arguments);
}

WidgetFoot.prototype = {
    ...
    render: function () {
        let _this = this;
        laytpl($("#module_footLayout").html()).render(_this.data, function (uiContent) {
            $(_this.parent).append(uiContent);
        });
        _this.currentWidget().css({top: _this.top, left: _this.left, zIndex: _this.zIndex});
        if (_this.editable) {
            laytpl($("#module_footLayout_menu").html()).render(_this.data, function (menubar) {
                _this.menubar[0] = menubar;
                $("#menu").append(_this.menubar[0])
                laytpl($("#module_footLayout_set").html()).render(_this.data, function (settingsContent) {
    ...
    loadTmpl: function (callback) {
        ...
        $("#template_search").load(`../footLayout/footLayout.html?_=${new Date().getTime()}`, function () {
            callback();
        });
    },
    /**
