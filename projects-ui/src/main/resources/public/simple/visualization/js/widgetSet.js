...
$('.widget_box').click(function(){
    ...
    // layerSetId = widgetId + 'Settings';
    activeWidgetBorder();
    $('#'+ widgetId + ' .widget_menu').show();
    var id = $(this).find('.widget_id').val();
    console.log('id',id)
    clickSet(widgetId + 'Settings'); //点击设置按钮弹出框
    deleteModule(widgetId); //点击删除按钮，删除组件
    return false;
});
...
function getModuleSetForm(){    
    ...
    var moduleId = $('#'+ widgetId).find('.widget_id').val(); //生成的组件id
    //获取表单设置的样式数据
    form.on('submit(setFormSubmit)', function(data){
    ...
    //获取表单设置的元数据数据
    form.on('submit(setFormTotalSubmit)', function(data){
...
function addModuleSet(params){
    ...
    ajax(service_prefix.visual,"/moduletable", "post", JSON.stringify(params)).then(function (data) {
        if (data.success) {
            ...
            selectSubGroupArr.push(subGroupObj);
            console.log('selectSubGroupArr',selectSubGroupArr);
        } else {
            console.log(data.msg);
        }
    });
