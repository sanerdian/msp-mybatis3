...
WidgetImage.prototype = {
    widgetId: 'widget_image_id',
    parent: "#app",
    type: 2,
    id:'',
    index: -1,
    ...
    width: 100, // 默认原始尺寸
    height: 100,
    content: '../upload/images/logo1.png',
    radius: 0,
    relatedDbTable: '',
    relatedDbField: '',
    ...
    render: function () {
        let _this = this;
        laytpl($("#module_image").html()).render(_this.data, function (uiContent) {
            $(_this.parent).append(uiContent);
        });
        _this.currentWidget().css({top: _this.top, left: _this.left, zIndex: _this.zIndex});
        if (_this.editable) {
            laytpl($("#module_image_menu").html()).render(_this.data, function (menubar) {
                _this.menubar[0] = menubar;
                $("#menu").append(_this.menubar[0])
                laytpl($("#module_image_set").html()).render(_this.data, function (settingsContent) {
    ...
    loadTmpl: function (callback) {
        $("#template").append("<div id='template_image'></div>");
        $("#template_image").load(`../module/image.html?_=${new Date().getTime()}`, function () {
            callback();
        });
    },
    /**
    ...
    uiStyleInit: function () {
        ...
        let img = content.children('img');
        if (_this.width > 0) {
            img.css({"width": _this.width});
        }
        if (_this.height > 0) {
            img.css({"height": _this.height});
        }
        if (_this.width === 0) {
            img.css({"width": ""});
        }
        ...
        content.attr('src', _this.content);
        if (_this.radius) {
            img.css({borderRadius: _this.radius + "px"});
        }
    },
    /**
     * 初始化用户界面
    ...
    settingsInit: function () {
        ...
        // image_picker
        upload.render({
            elem: "#" + _this.widgetId + "_settings" + ' .img_picker'
            , url: 'http://36.138.169.165:8081/member/menu/uploadExcel'
            , exts: 'jpg|png|svg'
            , size: 1024
            , accept: 'file'
            , acceptMime: 'image/*'
            , before: function (obj) {
                obj.preview(function(index, file, result){
                    console.log(398,result)
                   console.log(839,$(this)[0].WidgetImage.prototype.content);
                   $(this)[0].WidgetImage.prototype.content=result
            ...
            , done: function (res, index, upload) {
                console.log(22,res);
                if (res.code === 0) {
                    _this.select2('.img_picker').css({background: `url(${res.src})`})
    ...
    jsCode() {
        return `
        /** __start_widget_image_js_code__ **/
        (function(widgetId) {
            let __this = function () {
            };
    ...
    moduleAdd:function(){
        ...
        ajax('','/visual/componentpicture/add','post',JSON.stringify(jsonData)).then(function (data){
            if(data.success){			
    ...
    moduleEdit:function(){
        ...
        ajax('','/visual/componentpicture/'+_this.id,'PUT',JSON.stringify(jsonData)).then(function (data){
            if(data.success){					
                layer.msg('修改成功');
            }else{
                console.log(data.msg);
    ...
    moduleDelete:function (){
        let _this = this;
        ajax('','/visual/componentpicture/'+_this.id,'DELETE').then(function (data){
            if(data.success){					
                layer.msg('删除成功');
            }else{
                console.log(data.msg);
