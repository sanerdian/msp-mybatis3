...
function WidgetSearch() {
    this.dbTable = '';
    this.dbField = [{title: '姓名', field: 'name'}, {title: '手机号', field: 'phone'}];
...
WidgetSearch.prototype = {
    ...
    render: function () {
        let _this = this;
        laytpl($("#module_search").html()).render(_this.data, function (uiContent) {
            $(_this.parent).append(uiContent);
        });
        _this.currentWidget().css({top: _this.top, left: _this.left, zIndex: _this.zIndex});
        if (_this.editable) {
            laytpl($("#module_search_menu").html()).render(_this.data, function (menubar) {
                _this.menubar[0] = menubar;
                $("#menu").append(_this.menubar[0])
                laytpl($("#module_search_set").html()).render(_this.data, function (settingsContent) {
    ...
    loadTmpl: function (callback) {
        ...
        $("#template_search").load(`../module/search.html?_=${new Date().getTime()}`, function () {
            callback();
        });
    },
    /**
