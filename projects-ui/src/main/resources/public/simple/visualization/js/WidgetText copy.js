...
WidgetText.prototype = {
    widgetId: 'widget_text_id',
    parent: "#app",
    type: 1,
    index: -1,
    left: 50.0,
    ...
    render: function () {
        ...
        laytpl($("#module_text").html()).render(_this.data, function (uiContent) {
            $(_this.parent).append(uiContent);
        });
        if (_this.editable) {
            laytpl($("#module_text_menu").html()).render(_this.data, function (menubar) {
                _this.menubar[0] = menubar;
                $("#menu").append(_this.menubar[0])
                laytpl($("#module_text_set").html()).render(_this.data, function (settingsContent) {
    ...
    loadTmpl: function (callback) {
        $("#template").append("<div id='template_text'></div>");
        $("#template_text").load(`../module/text.html?_=${new Date().getTime()}`, function () {
            callback();
        });
    },
    /**
