/*__start_WidgetText__*/
/**
 * 文本组件
 */
 function WidgetTextTest() {
    this.create.apply(this, arguments);
}

WidgetTextTest.prototype = {
    widgetId: 'widget_test_text_id',
    parent: "#app",
    type: 1,
    index: -1,
    left: 50.0,
    ...
    render: function () {
        ...
        laytpl($("#module_test_text").html()).render(_this.data, function (uiContent) {
            $(_this.parent).append(uiContent);
        });
        if (_this.editable) {
            laytpl($("#module_test_text_menu").html()).render(_this.data, function (menubar) {
                _this.menubar[0] = menubar;
                $("#menu").append(_this.menubar[0])
                laytpl($("#module_test_text_set").html()).render(_this.data, function (settingsContent) {
    ...
    loadTmpl: function (callback) {
        $("#template").append("<div id='template_test_text'></div>");
        $("#template_test_text").load(`../testText/testText.html?_=${new Date().getTime()}`, function () {
            callback();
        });
    },
    /**
