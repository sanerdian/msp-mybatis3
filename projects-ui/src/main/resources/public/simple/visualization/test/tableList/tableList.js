/*__start_WidgetTableList__*/
/**
 * 表格列表组件
 */
 function tableListWidget() {
    console.log(333,this);
    this.create.apply(this, arguments);
}

tableListWidget.prototype = {
    widgetId: 'widget_table_list_id',
    parent: "#app",
    type: 1,
    index: -1,
    left: 50.0,
    ...
    render: function () {
        ...
        laytpl($("#module_table_list").html()).render(_this.data, function (uiContent) { //与html里面的ID一致module_text 
            $(_this.parent).append(uiContent);
        });
        if (_this.editable) {
            laytpl($("#module_table_list_menu").html()).render(_this.data, function (menubar) {
                _this.menubar[0] = menubar;
                $("#menu").append(_this.menubar[0])
                laytpl($("#module_table_list_set").html()).render(_this.data, function (settingsContent) {
    ...
    loadTmpl: function (callback) {
        $("#template").append("<div id='template_table_list'></div>");
        $("#template_table_list").load(`../test/tablelist/tablelist.html?_=${new Date().getTime()}`, function () {
            callback();
        });
    },
    /**
...
/*__end_WidgetTableList__*/

/*组件的点击事件 开始 */
// $("#tool_widget_table_list").click(function () {
//     let widget = new WidgetTableList(getParent());
//     if (widget.widgetId) {
