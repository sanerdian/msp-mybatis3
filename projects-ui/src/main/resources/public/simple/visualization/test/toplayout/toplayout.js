/*__start_WidgetTopLayout__*/
/**
 * 用户信息组件
 */
 function topLayoutWidget() {
    this.create.apply(this, arguments);
}

topLayoutWidget.prototype = {
    widgetId: 'widget_top_layout_id',
    parent: "#app",
    type: 1,
    index: -1,
    left: 50.0,
    ...
    render: function () {
        ...
        laytpl($("#module_top_layout").html()).render(_this.data, function (uiContent) { //与html里面的ID一致module_text 
            $(_this.parent).append(uiContent);
        });
        if (_this.editable) {
            laytpl($("#module_top_layout_menu").html()).render(_this.data, function (menubar) {
                _this.menubar[0] = menubar;
                $("#menu").append(_this.menubar[0])
                laytpl($("#module_top_layout_set").html()).render(_this.data, function (settingsContent) {
    ...
    loadTmpl: function (callback) {
        $("#template").append("<div id='template_top_layout'></div>");
        $("#template_top_layout").load(`../test/toplayout/toplayout.html?_=${new Date().getTime()}`, function () {
            callback();
        });
    },
    /**
...
/*__end_WidgetTopLayout__*/


 /*组件的点击事件 开始 */
//  $("#tool_widget_user").click(function () {
//     let widget = new WidgetTopLayout(getParent());
//     if (widget.widgetId) {
