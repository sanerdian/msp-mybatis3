...
public class CheckIdCard {
    ...
    public static void validate(String code){
        if(ObjectUtils.isEmpty(code) || ObjectUtils.isEmpty(code.trim())){
            throw new RuntimeException("身份证号码不可为空");
        }
        if(code.trim().length()!=15 && code.trim().length()!=18){
            throw new RuntimeException("身份证号码需为15或18位");
        }
        CheckIdCard checkIdCard = new CheckIdCard(code);
     ...
     * 验证身份证号和出生日期
     * @param code  身份证号
     * @param birthDay 出生日期：格式：2019-05-06
     */
    public static void validateBirthDay(String code, String birthDay){
        CheckIdCard checkIdCard = new CheckIdCard(code);
        String birthDayPart = checkIdCard.getBirthDayPart();
        String birth = birthDay.replaceAll("-", "");
        if(!birthDayPart.equals(birth)){
            throw new RuntimeException("身份证号与出生日期不相符");
        }
    }
     ...
     * @param sex 性别【男，女】
     */
    public static void validateSex(String code,String sex){
        CheckIdCard checkIdCard = new CheckIdCard(code);
        if("男".equals(sex)&&checkIdCard.isFemal()){
            throw new RuntimeException("身份证号与性别不相符");
        }
        if("女".equals(sex)&&checkIdCard.isMale()){
            throw new RuntimeException("身份证号与性别不相符");
        }
    }
