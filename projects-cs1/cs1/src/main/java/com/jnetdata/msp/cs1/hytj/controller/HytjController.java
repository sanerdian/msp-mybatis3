package com.jnetdata.msp.cs1.hytj.controller;
 ...
 */
@Controller
@RequestMapping("/cs1/hytj")
public class HytjController extends BaseController<Long,Hytj> {

    final private HytjService hytjService;
    ...
    @Autowired
    public HytjController(HytjService hytjService) {
        this.hytjService = hytjService;
    }
    ...
    @PostMapping
    @ResponseBody
    public JsonResult<EntityId<Long>> add(@Validated @RequestBody Hytj entity) {
    ...
    @PostMapping("/batch")
    @ResponseBody
    public JsonResult addbatch( @RequestBody List<Hytj> list) {
    ...
    @GetMapping("/{id}")
    @ResponseBody
    public JsonResult<Hytj> getById(@PathVariable("id") Long id) {
    ...
    @GetMapping("/pub/{id}")
    @ResponseBody
    public JsonResult<Hytj> pub(@PathVariable("id") Long id) {
        pubData(id);
        return renderSuccess();
    }
    ...
    @PostMapping("/pub")
    @ResponseBody
    public JsonResult<Hytj> pubByIds(@RequestBody Long[] ids) {
        for (Long id : ids) {
            pubData(id);
        }
        return renderSuccess();
    ...
    @GetMapping("/repub/{id}")
    @ResponseBody
    public JsonResult<Hytj> repub(@PathVariable("id") Long id) {
        repubData(id);
        return renderSuccess();
    }
    ...
    @PostMapping("/repub")
    @ResponseBody
    public JsonResult<Hytj> repubByIds(@RequestBody Long[] ids) {
        for (Long id : ids) {
            repubData(id);
        }
        return renderSuccess();
    ...
    @GetMapping("/pass/{id}")
    @ResponseBody
    public JsonResult<Hytj> pass(@PathVariable("id") Long id,String content) {
        passData(id,content);
        return renderSuccess();
    }
    ...
    @PostMapping("/pass")
    @ResponseBody
    public JsonResult<Hytj> passByIds(@RequestBody Long[] ids,String content) {
        for (Long id : ids) {
            passData(id,content);
        }
        return renderSuccess();
    ...
    @ResponseBody
    public JsonResult<Hytj> notpass(@PathVariable("id") Long id,String content) {
    ...
    @PostMapping("/notpass")
    @ResponseBody
    public JsonResult<Hytj> notpassByIds(@RequestBody Long[] ids,String content) {
        for (Long id : ids) {
            notpassData(id,content);
        }
        return renderSuccess();
    ...
    public void passData(Long id,String content){
        Hytj res = getService().getById(id);
        res.setStatus(10);
    ...
    public void pubData(Long id){
        ...
        if(!tocopycolumnId.isEmpty()){
            List<Hytj> toSyncEdit = getService().list(new PropertyWrapper<>(Hytj.class).eq("copyfromid", id).in("columnid", tocopycolumnId));
            if(!toSyncEdit.isEmpty()){
                for (Hytj sync : toSyncEdit) {
    ...
    public JsonResult<Void> deleteBatchIdsGet(@PathVariable("ids") String ids) {
        ...
        //删除已发布的
        List<Hytj> entitys = getService().listByIds(Arrays.asList(idss));
    ...
    public JsonResult<Void> deleteBatchIds(@RequestBody Long[] ids) {
        //删除已发布的
        List<Hytj> entitys = getService().listByIds(Arrays.asList(ids));
        List<Long> delIds = entitys.stream().filter(m -> m.getDocchannelid() != null).map(m -> m.getDocchannelid()).collect(Collectors.toList());
        getService().deleteBatchIds(delIds);
    ...
    public JsonResult<Void> batchdelIdsGet(@PathVariable("ids") String ids,@PathVariable("columnid") Long columnid) {
        ...
        //删除同步的
        List<Hytj> delSync = new ArrayList<>();
        for (Hytj entity : entitys) {
    ...
    */
    ...
    public void delSync(Long columnid,List<Hytj> entitys) {
        for (Hytj entity : entitys) {
    ...
    @PostMapping("/{id}")
    @ResponseBody
    public JsonResult<Void> updateByIdPost(@PathVariable("id") Long id, @RequestBody Hytj entity) {
        List<Fenfa> fenfas = fenfaService.list(new PropertyWrapper<>(Fenfa.class)
        ...
        if(targetColumnIds.size() > 0){
            List<Hytj> syncs = getService().list(new PropertyWrapper<>(Hytj.class).in("columnid", targetColumnIds).eq("copyfromid", id));
            if(syncs.size() > 0) {
                for (Hytj sync : syncs) {
                    Long syncId = sync.getId();
                    Long columnId = sync.getColumnid();
    ...
    @PutMapping("/{id}")
    @ResponseBody
    public JsonResult<Void> updateById(@PathVariable("id") Long id, @RequestBody Hytj entity) {
        List<Fenfa> fenfas = fenfaService.list(new PropertyWrapper<>(Fenfa.class)
            .like("fromids", entity.getColumnid() + "%")
            .eq("synctype", 0)
            .like("syncwhile", "1"));
    ...
    @ResponseBody
    public JsonResult<Void> batchUpdatePost(@RequestBody List<Hytj> list) {
    ...
    @ResponseBody
    public JsonResult<Void> batchUpdate(@RequestBody List<Hytj> list) {
        getService().updateBatchById(list);
        return renderSuccess();
    }
    ...
    @ResponseBody
    public JsonResult<Void> doUpdateAllColumnByIdPost(@PathVariable("id") Long id, @RequestBody Hytj entity) {
    ...
    @ResponseBody
    public JsonResult<Void> doUpdateAllColumnById(@PathVariable("id") Long id, @RequestBody Hytj entity) {
        return doUpdateAllColumnById(getService(), id, entity);
    }
    ...
    @GetMapping("/{id}/join")
    @ResponseBody
    public JsonResult<Hytj> getjoin(@PathVariable("id") Long id) {
        Hytj entity = getService().getEntityAndJoinsById(id);
    ...
    @ResponseBody
    public JsonResult<Pager<Hytj>> listjoin(@RequestBody BaseVo<Hytj> vo) {
        JsonResult<Pager<Hytj>> pagerJsonResult = doList(getService(), vo.getPager(), vo.getEntity());
        List<Hytj> list = pagerJsonResult.getObj().getRecords();
    ...
    public JsonResult importData(String columnid, MultipartFile file) throws Exception {
        ...
        List<Fieldinfo> list = fieldinfoUtilService.list(new PropertyWrapper<>(Fieldinfo.class).eq("tablename", "jmeta_hytj"));
        ...
        List<Hytj> list2 = new ArrayList<>();
        list1.forEach(item -> {
            Hytj obj = MapUtil.map2Obj(item, Hytj.class);
    ...
    @ResponseBody
    public JsonResult<Pager<Hytj>> list(@ModelAttribute Hytj entity,@RequestBody PageVo1 pager) {
    ...
    @ResponseBody
    public JsonResult<Pager<Hytj>> list(@RequestBody BaseVo<Hytj> vo) {
        if(vo.getEntity() == null) vo.setEntity(new Hytj());
            vo.getEntity().setDocstatus(0);
            JsonResult<Pager<Hytj>> result = doList(getService(), vo.getPager(), vo.getEntity());
            for (Hytj record : result.getObj().getRecords()) {
    ...
    @ResponseBody
    public JsonResult<Pager<Hytj>> delListing(@RequestBody BaseVo<Hytj> vo) {
        if(vo.getEntity() == null) vo.setEntity(new Hytj());
    ...
    @ResponseBody
    public JsonResult<Pager<Hytj>> pubListing(@RequestBody BaseVo<Hytj> vo) {
                if(vo.getEntity() == null) vo.setEntity(new Hytj());
        List<Hytj> list = getService().list(new PropertyWrapper<>(Hytj.class).eq("status", "21").eq("docstatus", 0).select("docchannelid"));
    ...
    public void export(Hytj entity, ExportVo vo, HttpServletResponse response) {
        List<Hytj> datalist;
        if(vo.getArea() == 2){
        ...
        List<Fieldinfo> list = fieldinfoUtilService.list(new PropertyWrapper<>(Fieldinfo.class).eq("tablename", "jmeta_hytj").eq("showList", 1));
        DownMetadataUtil.downMetadata(list,datalist,response,vo.getExt(),vo.getIncludeTitle(),Hytj.class);
    }

    @ApiOperation(value = "导出", notes="根据vo指定条件进行导出")
    @PostMapping(value = "/export2")
    public void export2(@RequestBody ExportVo<Hytj> vo, HttpServletResponse response) {
        List<Hytj> datalist;
        if(vo.getArea() == 2){
            datalist = getService().list();
        }else{
    ...
    public void export3(ExportVo<Hytj> vo, HttpServletResponse response) {
        List<Hytj> datalist;
        if(vo.getArea() == 2){
            datalist = getService().list();
        }else{
        ...
        if(vo.getFields() != null){
            DownMetadataUtil.downMetadata(datalist,response,vo.getExt(),vo.getIncludeTitle(),Hytj.class,vo.getFields());
        }else{
            List<Fieldinfo> list = fieldinfoUtilService.list(new PropertyWrapper<>(Fieldinfo.class).eq("tablename","jmeta_hytj").eq("showList", 1));
            DownMetadataUtil.downMetadata(list,datalist,response,vo.getExt(),vo.getIncludeTitle(),Hytj.class);
        }
    ...
    private HytjService getService() {
        return this.hytjService;
    }
