package com.jnetdata.msp.cs1.middle.controller;
 ...
 */
@Controller
@RequestMapping("/cs1/middle")
public class MiddleController extends BaseController<Long,Middle> {

    final private MiddleService middleService;
    ...
    @PostMapping
    @ResponseBody
    public JsonResult<EntityId<Long>> add(@Validated @RequestBody Middle entity) {
    ...
    @PostMapping("/batch")
    @ResponseBody
    public JsonResult addbatch( @RequestBody List<Middle> list) {
    ...
    @GetMapping("/{id}")
    @ResponseBody
    public JsonResult<Middle> getById(@PathVariable("id") Long id) {
    ...
    @GetMapping("/pub/{id}")
    @ResponseBody
    public JsonResult<Middle> pub(@PathVariable("id") Long id) {
        pubData(id);
        return renderSuccess();
    }
    ...
    @PostMapping("/pub")
    @ResponseBody
    public JsonResult<Middle> pubByIds(@RequestBody Long[] ids) {
        for (Long id : ids) {
            pubData(id);
        }
        return renderSuccess();
    ...
    @GetMapping("/repub/{id}")
    @ResponseBody
    public JsonResult<Middle> repub(@PathVariable("id") Long id) {
        repubData(id);
        return renderSuccess();
    }
    ...
    @PostMapping("/repub")
    @ResponseBody
    public JsonResult<Middle> repubByIds(@RequestBody Long[] ids) {
        for (Long id : ids) {
            repubData(id);
        }
        return renderSuccess();
    ...
    @GetMapping("/pass/{id}")
    @ResponseBody
    public JsonResult<Middle> pass(@PathVariable("id") Long id,String content) {
        passData(id,content);
        return renderSuccess();
    }
    ...
    @PostMapping("/pass")
    @ResponseBody
    public JsonResult<Middle> passByIds(@RequestBody Long[] ids,String content) {
        for (Long id : ids) {
            passData(id,content);
        }
        return renderSuccess();
    ...
    @ResponseBody
    public JsonResult<Middle> notpass(@PathVariable("id") Long id,String content) {
    ...
    @PostMapping("/notpass")
    @ResponseBody
    public JsonResult<Middle> notpassByIds(@RequestBody Long[] ids,String content) {
        for (Long id : ids) {
            notpassData(id,content);
        }
        return renderSuccess();
    ...
    public void passData(Long id,String content){
        Middle res = getService().getById(id);
        res.setStatus(10);
    ...
    public void pubData(Long id){
        ...
        if(!tocopycolumnId.isEmpty()){
            List<Middle> toSyncEdit = getService().list(new PropertyWrapper<>(Middle.class).eq("copyfromid", id).in("columnid", tocopycolumnId));
            if(!toSyncEdit.isEmpty()){
                for (Middle sync : toSyncEdit) {
    ...
    public JsonResult<Void> deleteBatchIdsGet(@PathVariable("ids") String ids) {
        ...
        //删除已发布的
        List<Middle> entitys = getService().listByIds(Arrays.asList(idss));
        ...
        List<Middle> toDel = new ArrayList<>();
        for (String s : idss) {
            Middle entity = new Middle();
    ...
    public JsonResult<Void> deleteBatchIds(@RequestBody Long[] ids) {
        //删除已发布的
        List<Middle> entitys = getService().listByIds(Arrays.asList(ids));
        List<Long> delIds = entitys.stream().filter(m -> m.getDocchannelid() != null).map(m -> m.getDocchannelid()).collect(Collectors.toList());
        getService().deleteBatchIds(delIds);
    ...
    public JsonResult<Void> batchdelIdsGet(@PathVariable("ids") String ids,@PathVariable("columnid") Long columnid) {
        ...
        //删除同步的
        List<Middle> delSync = new ArrayList<>();
        for (Middle entity : entitys) {
    ...
    */
    ...
    public void delSync(Long columnid,List<Middle> entitys) {
        for (Middle entity : entitys) {
    ...
    @PostMapping("/{id}")
    @ResponseBody
    public JsonResult<Void> updateByIdPost(@PathVariable("id") Long id, @RequestBody Middle entity) {
        List<Fenfa> fenfas = fenfaService.list(new PropertyWrapper<>(Fenfa.class)
        ...
        if(targetColumnIds.size() > 0){
            List<Middle> syncs = getService().list(new PropertyWrapper<>(Middle.class).in("columnid", targetColumnIds).eq("copyfromid", id));
            if(syncs.size() > 0) {
                for (Middle sync : syncs) {
                    Long syncId = sync.getId();
                    Long columnId = sync.getColumnid();
    ...
    @PutMapping("/{id}")
    @ResponseBody
    public JsonResult<Void> updateById(@PathVariable("id") Long id, @RequestBody Middle entity) {
        List<Fenfa> fenfas = fenfaService.list(new PropertyWrapper<>(Fenfa.class)
            .like("fromids", entity.getColumnid() + "%")
            .eq("synctype", 0)
            .like("syncwhile", "1"));
    ...
    @ResponseBody
    public JsonResult<Void> batchUpdatePost(@RequestBody List<Middle> list) {
    ...
    @ResponseBody
    public JsonResult<Void> batchUpdate(@RequestBody List<Middle> list) {
        getService().updateBatchById(list);
        return renderSuccess();
    }
    ...
    @ResponseBody
    public JsonResult<Void> doUpdateAllColumnByIdPost(@PathVariable("id") Long id, @RequestBody Middle entity) {
    ...
    @ResponseBody
    public JsonResult<Void> doUpdateAllColumnById(@PathVariable("id") Long id, @RequestBody Middle entity) {
        return doUpdateAllColumnById(getService(), id, entity);
    }
    ...
    @GetMapping("/{id}/join")
    @ResponseBody
    public JsonResult<Middle> getjoin(@PathVariable("id") Long id) {
        Middle entity = getService().getEntityAndJoinsById(id);
    ...
    @ResponseBody
    public JsonResult<Pager<Middle>> listjoin(@RequestBody BaseVo<Middle> vo) {
        JsonResult<Pager<Middle>> pagerJsonResult = doList(getService(), vo.getPager(), vo.getEntity());
        List<Middle> list = pagerJsonResult.getObj().getRecords();
    ...
    public JsonResult importData(String columnid, MultipartFile file) throws Exception {
        ...
        List<Fieldinfo> list = fieldinfoUtilService.list(new PropertyWrapper<>(Fieldinfo.class).eq("tablename", "jmeta_middle"));
        ...
        List<Middle> list2 = new ArrayList<>();
        list1.forEach(item -> {
            Middle obj = MapUtil.map2Obj(item, Middle.class);
    ...
    @ResponseBody
    public JsonResult<Pager<Middle>> list(@ModelAttribute Middle entity,@RequestBody PageVo1 pager) {
    ...
    @ResponseBody
    public JsonResult<Pager<Middle>> list(@RequestBody BaseVo<Middle> vo) {
        if(vo.getEntity() == null) vo.setEntity(new Middle());
            vo.getEntity().setDocstatus(0);
            JsonResult<Pager<Middle>> result = doList(getService(), vo.getPager(), vo.getEntity());
            for (Middle record : result.getObj().getRecords()) {
    ...
    @ResponseBody
    public JsonResult<Pager<Middle>> delListing(@RequestBody BaseVo<Middle> vo) {
        if(vo.getEntity() == null) vo.setEntity(new Middle());
    ...
    @ResponseBody
    public JsonResult<Pager<Middle>> pubListing(@RequestBody BaseVo<Middle> vo) {
                if(vo.getEntity() == null) vo.setEntity(new Middle());
        List<Middle> list = getService().list(new PropertyWrapper<>(Middle.class).eq("status", "21").eq("docstatus", 0).select("docchannelid"));
    ...
    public void export(Middle entity, ExportVo vo, HttpServletResponse response) {
        List<Middle> datalist;
        if(vo.getArea() == 2){
            datalist = getService().list();
        }else{
            datalist = getService().list(createCondition(entity));
        }
        List<Fieldinfo> list = fieldinfoUtilService.list(new PropertyWrapper<>(Fieldinfo.class).eq("tablename", "jmeta_middle").eq("showList", 1));
        DownMetadataUtil.downMetadata(list,datalist,response,vo.getExt(),vo.getIncludeTitle(),Middle.class);
    }

    @ApiOperation(value = "导出", notes="根据vo指定条件进行导出")
    @PostMapping(value = "/export2")
    public void export2(@RequestBody ExportVo<Middle> vo, HttpServletResponse response) {
        List<Middle> datalist;
        if(vo.getArea() == 2){
            datalist = getService().list();
        }else{
    ...
    public void export3(ExportVo<Middle> vo, HttpServletResponse response) {
        List<Middle> datalist;
        if(vo.getArea() == 2){
            datalist = getService().list();
        }else{
        ...
        if(vo.getFields() != null){
            DownMetadataUtil.downMetadata(datalist,response,vo.getExt(),vo.getIncludeTitle(),Middle.class,vo.getFields());
        }else{
            List<Fieldinfo> list = fieldinfoUtilService.list(new PropertyWrapper<>(Fieldinfo.class).eq("tablename","jmeta_middle").eq("showList", 1));
            DownMetadataUtil.downMetadata(list,datalist,response,vo.getExt(),vo.getIncludeTitle(),Middle.class);
        }
    ...
    private MiddleService getService() {
