...
import com.jnetdata.msp.dict3.wordname.mapper.DictFanyiciMapper;
...
@RestController
@RequestMapping("/dictfanyici")
public class DictFanyiciController {



    @Autowired
    DictFanyiciMapper dictFanyiciMapper;
    ...
    public JsonResult list(@RequestBody BaseVo<DictFanyici> vo){
//
        ...
        QueryWrapper<DictFanyici> wrapper=new QueryWrapper();
        ...
        Page<DictFanyici> page = new Page<>();
        page.setSize(page_size);
        page.setCurrent(current);

        List<DictFanyici> dictWords= dictFanyiciMapper.selectPage(page,wrapper).getRecords() ;
         int total  =  dictFanyiciMapper.selectCount(wrapper) ;
    ...
    public JsonResult view(@PathVariable("id") Long id){
        ...
        DictFanyici obj =    dictFanyiciMapper.selectById(id) ;
    ...
    public JsonResult add(@RequestBody DictFanyici vo){
//

        val user = SessionUser.getCurrentUser();

        vo.setJmetafanyiciid(BigInteger.valueOf(ID.getId()));
        vo.setCruser(user.getName());
        vo.setCrtime(new Date());
    int result   =  dictFanyiciMapper.insert(vo) ;
    ...
    public JsonResult modify(@PathVariable("id") Long id,@RequestBody DictFanyici vo){

//        vo.setJmetafanyiciid(id);
        val user = SessionUser.getCurrentUser();
        vo.setOpertime(new Date());
        vo.setOperuser( user.getName() );

        vo.setJmetafanyiciid(BigInteger.valueOf(id));
        dictFanyiciMapper.updateById(vo);
...
//
       String[] arr = vo.getField1().split(",") ;
        ...
        int result   =  dictFanyiciMapper.deleteBatchIds(idList) ;
...
//        int result   =  dictFanyiciMapper.deleteById(id) ;


//        Wrapper<DictFanyici> var1 = new Wrapper<DictFanyici>() {
//            @Override
//            public String getSqlSegment() {
//                return null;
//            }
//        };
//        var1.eq("JMETAFANYICIID",id) ;


//        int result   =  dictFanyiciMapper.delete(var1);
        int result   =  dictFanyiciMapper.deleteById(id);
        return JsonResult.success(result);
    }
