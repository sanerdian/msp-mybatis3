...
angular.module('flowableModeler')
    .controller('ProcessesCtrl', ['$rootScope', '$scope', '$translate', '$http', '$timeout','$location', '$modal', function ($rootScope, $scope, $translate, $http, $timeout, $location, $modal) {
        ...
        $scope.pushProcess = function() {
            if (!$scope.radioVal) {
                alert("请选择相应流程")
                return
            }
            $http({method: 'POST', url: FLOWABLE.CONFIG.contextRoot+'/flowable/deploy', data: {"modelId":$scope.radioVal}}).
            success(function(data, status, headers, config) {
                alert(data.msg)
            }).
            error(function(data, status, headers, config) {
        ...
        //   设置元数据栏目
        $scope.settingProcess = function() {
            _internalCreateModal({
                template: 'views/popup/process-set-column.html?version=' + Date.now()
            }, $modal, $scope);
        };

        $scope.selectProcess = function(value) {
            console.log("选择元数据栏目",value);
...
angular.module('flowableModeler')
    .controller('CreateNewProcessModelCtrl', ['$rootScope', '$scope', '$modal', '$http', '$location',function ($rootScope, $scope, $modal, $http, $location) {
        ...
        $scope.model = {
            ...
            process: {
                ...
                key: Date.now(),
                description: '',
                sort:document.getElementsByClassName('item-box').length+1,
                modelType: 0,
                processType:'固定流程',
                delegationSettings:'不允许委托',
                formClassification:'元数据表',
                ProcessAttachment:'开启',
                ProcessClassification:'行政办公'
            }
        };
        ...
        $scope.processTypeOptions=[
            {name:'固定流程',value:'固定流程'}
        ]
        // 委托设置选项
        $scope.delegationSettingsOptions=[
            {name:'允许委托',value:'允许委托'}
            ,{name:'不允许委托',value:'不允许委托'}
        ]
        // 表单分类选项
        $scope.formClassOptions=[
            {name:'元数据表',value:'元数据表'}
            ,{name:'元数据栏目',value:'元数据栏目'}
        ]
        // 流程附件选项
        $scope.formAttachmentOptions=[
            {name:'开启',value:'开启'}
            ,{name:'关闭',value:'关闭'}
        ]

        // 流程分类选项
        $scope.processClassOptions=[
            {name:'行政办公',value:'行政办公'}
            ,{name:'自动采购',value:'自动采购'}
            ,{name:'法务相关',value:'法务相关'}
            ,{name:'费用报销',value:'费用报销'}
            ,{name:'人事管理',value:'人事管理'}
        ]



        // 监听表单分类选项内容
        $scope.$watch('model.process.formClassification',function (newValue, oldValue) {
            // 默认进来不触发
            if ($scope.model.first) {
                $scope.model.first = false
                return
            }
            if (newValue == '元数据表') {
                _internalCreateModal({
                    template: 'views/popup/process-metadata-table.html?version=' + Date.now()
                }, $modal, $scope);
            }else if (newValue == '元数据栏目') {
                _internalCreateModal({
                    template: 'views/popup/process-metadata-column.html?version=' + Date.now()
                }, $modal, $scope);
            }
        });
...
angular.module('flowableModeler')
    .controller('SetColumnProcessModelCtrl', ['$rootScope', '$scope', '$modal', '$http', '$location', function ($rootScope, $scope, $modal, $http, $location) {
        $scope.model = {
            loading:false
        }
        $scope.ok = function () {
...
angular.module('flowableModeler')
    .controller('ProcessMetadataColumnModelCtrl', ['$rootScope', '$scope', '$modal', '$http', '$location',
...
angular.module('flowableModeler')
    .controller('ProcessMetadataTableModelCtrl', ['$rootScope', '$scope', '$modal', '$http', '$location',
        function ($rootScope, $scope, $modal, $http, $location) {
            ...
            $scope.model = {
                ...
                process: {
                    name: 'JMETA_',
                    processType:'PUBLISHDISTRIB',
                    delegationSettings:'NQI.一站式服务'
                }
            };
            ...
            // 表英文名称选项
            $scope.processTypeOptions=[
                {name:'PUBLISHDISTRIB',value:'PUBLISHDISTRIB'}
                ,{name:'RULE-EXEC-LOG',value:'RULE-EXEC-LOG'}
                ,{name:'RULE-PARAM',value:'RULE-PARAM'}
                ,{name:'RULE-QI-FACTOR',value:'RULE-QI-FACTOR'}
            ...
            $scope.delegationSettingsOptions=[
                ...
                ,{name:'通航',value:'通航'}
                ,{name:'测试',value:'测试'}
                ,{name:'NQI.一站式服务',value:'NQI.一站式服务'}
            ]
