...
public class GmapnetcacheController extends BaseController<Long,Gmapnetcache> {
    ...
    @Autowired
    public GmapnetcacheController(GmapnetcacheService gmapnetcacheService) {
    ...
    @PostMapping
    @ResponseBody
    public JsonResult<EntityId<Long>> add(@Validated @RequestBody Gmapnetcache entity) {
    ...
    public void syncAdd(Long columnid,Long fromid){
        Gmapnetcache toCopy = new Gmapnetcache();
        toCopy.setSingletempkate(0L);
        toCopy.setCopyfromid(fromid);

        Gmapnetcache toYinyong = new Gmapnetcache();
    ...
    @PostMapping("/batch")
    @ResponseBody
    public JsonResult addbatch( @RequestBody List<Gmapnetcache> list) {
    ...
    @GetMapping("/{id}")
    @ResponseBody
    public JsonResult<Gmapnetcache> getById(@PathVariable("id") Long id) {
    ...
    @GetMapping("/pub/{id}")
    @ResponseBody
    public JsonResult<Gmapnetcache> pub(@PathVariable("id") Long id) {
        pubData(id);
        return renderSuccess();
    }
    ...
    @PostMapping("/pub")
    @ResponseBody
    public JsonResult<Gmapnetcache> pubByIds(@RequestBody Long[] ids) {
        for (Long id : ids) {
            pubData(id);
        }
        return renderSuccess();
    ...
    @GetMapping("/repub/{id}")
    @ResponseBody
    public JsonResult<Gmapnetcache> repub(@PathVariable("id") Long id) {
        repubData(id);
        return renderSuccess();
    }
    ...
    @PostMapping("/repub")
    @ResponseBody
    public JsonResult<Gmapnetcache> repubByIds(@RequestBody Long[] ids) {
        for (Long id : ids) {
            repubData(id);
        }
        return renderSuccess();
    ...
    @GetMapping("/pass/{id}")
    @ResponseBody
    public JsonResult<Gmapnetcache> pass(@PathVariable("id") Long id,String content) {
        passData(id,content);
        return renderSuccess();
    }
    ...
    @PostMapping("/pass")
    @ResponseBody
    public JsonResult<Gmapnetcache> passByIds(@RequestBody Long[] ids,String content) {
        for (Long id : ids) {
            passData(id,content);
        }
        return renderSuccess();
    ...
    @ResponseBody
    public JsonResult<Gmapnetcache> notpass(@PathVariable("id") Long id,String content) {
    ...
    @PostMapping("/notpass")
    @ResponseBody
    public JsonResult<Gmapnetcache> notpassByIds(@RequestBody Long[] ids,String content) {
        for (Long id : ids) {
            notpassData(id,content);
        }
        return renderSuccess();
    ...
    public void passData(Long id,String content){
        Gmapnetcache res = getService().getById(id);
        res.setStatus(10);
    ...
    public void pubData(Long id){
        ...
        if(!tocopycolumnId.isEmpty()){
            List<Gmapnetcache> toSyncEdit = getService().list(new PropertyWrapper<>(Gmapnetcache.class).eq("copyfromid", id).in("columnid", tocopycolumnId));
            if(!toSyncEdit.isEmpty()){
                for (Gmapnetcache sync : toSyncEdit) {
    ...
    public JsonResult<Void> deleteByIdGet(@PathVariable("id") Long id) {
        ...
        Gmapnetcache entity = new Gmapnetcache();
        entity.setId(id);
    ...
    public JsonResult<Void> deleteBatchIdsGet(@PathVariable("ids") String ids) {
        ...
        //删除已发布的
        List<Gmapnetcache> entitys = getService().listByIds(Arrays.asList(idss));
        ...
        List<Gmapnetcache> toDel = new ArrayList<>();
        for (String s : idss) {
            Gmapnetcache entity = new Gmapnetcache();
    ...
    public JsonResult<Void> deleteBatchIds(@RequestBody Long[] ids) {
        //删除已发布的
        List<Gmapnetcache> entitys = getService().listByIds(Arrays.asList(ids));
        List<Long> delIds = entitys.stream().filter(m -> m.getDocchannelid() != null).map(m -> m.getDocchannelid()).collect(Collectors.toList());
        getService().deleteBatchIds(delIds);
    ...
    public JsonResult<Void> batchdelIdsGet(@PathVariable("ids") String ids,@PathVariable("columnid") Long columnid) {
        ...
        //删除同步的
        List<Gmapnetcache> delSync = new ArrayList<>();
        for (Gmapnetcache entity : entitys) {
    ...
    */
    ...
    public void delSync(Long columnid,List<Gmapnetcache> entitys) {
        for (Gmapnetcache entity : entitys) {
    ...
    @PostMapping("/{id}")
    @ResponseBody
    public JsonResult<Void> updateByIdPost(@PathVariable("id") Long id, @RequestBody Gmapnetcache entity) {
        List<Fenfa> fenfas = fenfaService.list(new PropertyWrapper<>(Fenfa.class)
        ...
        if(targetColumnIds.size() > 0){
            List<Gmapnetcache> syncs = getService().list(new PropertyWrapper<>(Gmapnetcache.class).in("columnid", targetColumnIds).eq("copyfromid", id));
            if(syncs.size() > 0) {
                for (Gmapnetcache sync : syncs) {
                    Long syncId = sync.getId();
                    Long columnId = sync.getColumnid();
    ...
    @PutMapping("/{id}")
    @ResponseBody
    public JsonResult<Void> updateById(@PathVariable("id") Long id, @RequestBody Gmapnetcache entity) {
        List<Fenfa> fenfas = fenfaService.list(new PropertyWrapper<>(Fenfa.class)
            .like("fromids", entity.getColumnid() + "%")
            .eq("synctype", 0)
            .like("syncwhile", "1"));
    ...
    @ResponseBody
    public JsonResult<Void> batchUpdatePost(@RequestBody List<Gmapnetcache> list) {
    ...
    @ResponseBody
    public JsonResult<Void> batchUpdate(@RequestBody List<Gmapnetcache> list) {
        getService().updateBatchById(list);
        return renderSuccess();
    }
    ...
    public JsonResult<Void> updateBatchById(@RequestBody Long[] ids, Integer docstatus) {
        List<Gmapnetcache> list = new ArrayList<>();
        for (Long id : ids) {
            Gmapnetcache entity = new Gmapnetcache();
            entity.setId(id);
    ...
    @ResponseBody
    public JsonResult<Void> doUpdateAllColumnByIdPost(@PathVariable("id") Long id, @RequestBody Gmapnetcache entity) {
    ...
    @ResponseBody
    public JsonResult<Void> doUpdateAllColumnById(@PathVariable("id") Long id, @RequestBody Gmapnetcache entity) {
        return doUpdateAllColumnById(getService(), id, entity);
    }
    ...
    @GetMapping("/{id}/join")
    @ResponseBody
    public JsonResult<Gmapnetcache> getjoin(@PathVariable("id") Long id) {
        Gmapnetcache entity = getService().getEntityAndJoinsById(id);
    ...
    @ResponseBody
    public JsonResult<Pager<Gmapnetcache>> listjoin(@RequestBody BaseVo<Gmapnetcache> vo) {
        JsonResult<Pager<Gmapnetcache>> pagerJsonResult = doList(getService(), vo.getPager(), vo.getEntity());
        List<Gmapnetcache> list = pagerJsonResult.getObj().getRecords();
    ...
    public JsonResult importData(String columnid, MultipartFile file) throws Exception {
        ...
        List<Fieldinfo> list = fieldinfoUtilService.list(new PropertyWrapper<>(Fieldinfo.class).eq("tablename", "jmeta_gmapnetcache"));
        ...
        List<Gmapnetcache> list2 = new ArrayList<>();
        list1.forEach(item -> {
            Gmapnetcache obj = MapUtil.map2Obj(item, Gmapnetcache.class);
    ...
    @ResponseBody
    public JsonResult<Pager<Gmapnetcache>> list(@ModelAttribute Gmapnetcache entity,@RequestBody PageVo1 pager) {
    ...
    @ResponseBody
    public JsonResult<Pager<Gmapnetcache>> list(@RequestBody BaseVo<Gmapnetcache> vo) {
        if(vo.getEntity() == null) vo.setEntity(new Gmapnetcache());
            vo.getEntity().setDocstatus(0);
            JsonResult<Pager<Gmapnetcache>> result = doList(getService(), vo.getPager(), vo.getEntity());
            for (Gmapnetcache record : result.getObj().getRecords()) {
    ...
    @ResponseBody
    public JsonResult<Pager<Gmapnetcache>> delListing(@RequestBody BaseVo<Gmapnetcache> vo) {
        if(vo.getEntity() == null) vo.setEntity(new Gmapnetcache());
    ...
    @ResponseBody
    public JsonResult<Pager<Gmapnetcache>> pubListing(@RequestBody BaseVo<Gmapnetcache> vo) {
                if(vo.getEntity() == null) vo.setEntity(new Gmapnetcache());
        List<Gmapnetcache> list = getService().list(new PropertyWrapper<>(Gmapnetcache.class).eq("status", "21").eq("docstatus", 0).select("docchannelid"));
    ...
    public void export(Gmapnetcache entity, ExportVo vo, HttpServletResponse response) {
        List<Gmapnetcache> datalist;
        if(vo.getArea() == 2){
            datalist = getService().list();
        }else{
            datalist = getService().list(createCondition(entity));
        }
        List<Fieldinfo> list = fieldinfoUtilService.list(new PropertyWrapper<>(Fieldinfo.class).eq("tablename", "jmeta_gmapnetcache").eq("showList", 1));
        DownMetadataUtil.downMetadata(list,datalist,response,vo.getExt(),vo.getIncludeTitle(),Gmapnetcache.class);
    }

    @ApiOperation(value = "导出", notes="根据vo指定条件进行导出")
    @PostMapping(value = "/export2")
    public void export2(@RequestBody ExportVo<Gmapnetcache> vo, HttpServletResponse response) {
        List<Gmapnetcache> datalist;
        if(vo.getArea() == 2){
            datalist = getService().list();
        }else{
    ...
    public void export3(ExportVo<Gmapnetcache> vo, HttpServletResponse response) {
        List<Gmapnetcache> datalist;
        if(vo.getArea() == 2){
            datalist = getService().list();
        }else{
        ...
        if(vo.getFields() != null){
            DownMetadataUtil.downMetadata(datalist,response,vo.getExt(),vo.getIncludeTitle(),Gmapnetcache.class,vo.getFields());
        }else{
            List<Fieldinfo> list = fieldinfoUtilService.list(new PropertyWrapper<>(Fieldinfo.class).eq("tablename","jmeta_gmapnetcache").eq("showList", 1));
            DownMetadataUtil.downMetadata(list,datalist,response,vo.getExt(),vo.getIncludeTitle(),Gmapnetcache.class);
        }
