package com.jnetdata.msp.tlujy.collect.controller;
...
import com.jnetdata.msp.tlujy.collect.service.CollectService;
 ...
 */
@Controller
@RequestMapping("/tlujy/collect")
public class CollectController extends BaseController<Long,Collect> {
    ...
    public JsonResult<EntityId<Long>> add(@Validated @RequestBody Collect entity) {
    ...
    @PutMapping("/{id}")
    @ResponseBody
    public JsonResult<Void> updateById(@PathVariable("id") Long id, @RequestBody Collect entity) {
    ...
    @ResponseBody
    public JsonResult<Void> doUpdateAllColumnById(@PathVariable("id") Long id, @RequestBody Collect entity) {
    ...
    @GetMapping("/{id}")
    @ResponseBody
    public JsonResult<Collect> getById(@PathVariable("id") Long id) {
    ...
    @GetMapping("/{id}/join")
    @ResponseBody
    public JsonResult<Collect> getjoin(@PathVariable("id") Long id) {
        Collect entity = getService().getEntityAndJoinsById(id);
    ...
    @ResponseBody
    public JsonResult<Pager<Collect>> listjoin(@RequestBody BaseVo<Collect> vo) {
        JsonResult<Pager<Collect>> pagerJsonResult = doList(getService(), vo.getPager(), vo.getEntity());
        List<Collect> list = pagerJsonResult.getObj().getRecords();
    ...
    @ResponseBody
    public JsonResult<Pager<Collect>> list(@ModelAttribute Collect entity,@RequestBody PageVo1 pager) {
    ...
    @ResponseBody
    public JsonResult<Pager<Collect>> list(@RequestBody BaseVo<Collect> vo) {
        return doList(getService(), vo.getPager(), vo.getEntity());
    }

    @ApiOperation(value = "导出", notes="根据vo指定条件进行导出")
    @PostMapping(value = "/export")
    public void export(Collect entity, ExportVo vo, HttpServletResponse response) {
        List<Collect> datalist;
        if(vo.getArea() == 2){
    ...
    private CollectService getService() {
