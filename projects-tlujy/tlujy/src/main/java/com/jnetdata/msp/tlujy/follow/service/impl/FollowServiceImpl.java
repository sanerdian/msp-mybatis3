package com.jnetdata.msp.tlujy.follow.service.impl;
 ...
 */
@Service
public class FollowServiceImpl extends BaseServiceImpl<FollowMapper, Follow> implements FollowService {


    private String andOr = "and";
    @Override
    public PropertyWrapper<Follow> makeSearchWrapper(Map<String, Object> condition) {
        if(condition.get("andOr")!=null) {
            this.andOr = condition.get("andOr").toString();
        }
        PropertyWrapper<Follow> pw = createWrapperUtil(condition)
                                        .eq("id")
                                                            .eq("docchannelid")
                                                            .eq("docstatus")
                                                            .eq("singletempkate")
    ...
    @Override
    public Follow getEntityAndJoinsById(Long id){
        Follow entity = getById(id);
        getJoin(entity, null);
        return entity;
    }

    public void getJoin(Follow entity, Follow vo) {
        if(vo == null) {
            vo = new Follow();
        }
    }
    ...
    @Override
    public void getListJoin(List<Follow> list, Follow vo) {
        for (Follow entity : list) {
            getJoin(entity, vo);
        }
    }
