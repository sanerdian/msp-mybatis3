package com.jnetdata.msp.tlujy.fjtp.service.impl;

import com.jnetdata.msp.tlujy.fjtp.model.Fjtp;
import com.jnetdata.msp.tlujy.fjtp.mapper.FjtpMapper;
 ...
 */
@Service
public class FjtpServiceImpl extends BaseServiceImpl<FjtpMapper, Fjtp> implements FjtpService {


    private String andOr = "and";
    @Override
    public PropertyWrapper<Fjtp> makeSearchWrapper(Map<String, Object> condition) {
        if(condition.get("andOr")!=null) {
            this.andOr = condition.get("andOr").toString();
        }
        PropertyWrapper<Fjtp> pw = createWrapperUtil(condition)
                                        .eq("id")
                                                            .eq("docchannelid")
                                                            .eq("docstatus")
                                                            .eq("singletempkate")
    ...
    @Override
    public Fjtp getEntityAndJoinsById(Long id){
        Fjtp entity = getById(id);
        getJoin(entity, null);
        return entity;
    }

    public void getJoin(Fjtp entity, Fjtp vo) {
        if(vo == null) {
            vo = new Fjtp();
        }
    }
    ...
    @Override
    public void getListJoin(List<Fjtp> list, Fjtp vo) {
        for (Fjtp entity : list) {
            getJoin(entity, vo);
        }
    }
