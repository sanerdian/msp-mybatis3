package com.jnetdata.msp.tlujy.yjfk_photo.controller;
 ...
 */
@Controller
@RequestMapping("/tlujy/yjfkphoto")
public class YjfkPhotoController extends BaseController<Long,YjfkPhoto> {

    final private YjfkPhotoService yjfkPhotoService;
    ...
    @Autowired
    public YjfkPhotoController(YjfkPhotoService yjfkPhotoService) {
        this.yjfkPhotoService = yjfkPhotoService;
    }
    ...
    public JsonResult<EntityId<Long>> add(@Validated @RequestBody YjfkPhoto entity) {
    ...
    @PutMapping("/{id}")
    @ResponseBody
    public JsonResult<Void> updateById(@PathVariable("id") Long id, @RequestBody YjfkPhoto entity) {
    ...
    public JsonResult<Void> updateBatchById(@RequestBody Long[] ids, Integer docstatus) {
        List<YjfkPhoto> list = new ArrayList<>();
        for (Long id : ids) {
        YjfkPhoto entity = new YjfkPhoto();
        entity.setId(id);
    ...
    @ResponseBody
    public JsonResult<Void> doUpdateAllColumnById(@PathVariable("id") Long id, @RequestBody YjfkPhoto entity) {
    ...
    @GetMapping("/{id}")
    @ResponseBody
    public JsonResult<YjfkPhoto> getById(@PathVariable("id") Long id) {
    ...
    @GetMapping("/{id}/join")
    @ResponseBody
    public JsonResult<YjfkPhoto> getjoin(@PathVariable("id") Long id) {
        YjfkPhoto entity = getService().getEntityAndJoinsById(id);
    ...
    @ResponseBody
    public JsonResult<Pager<YjfkPhoto>> listjoin(@RequestBody BaseVo<YjfkPhoto> vo) {
        JsonResult<Pager<YjfkPhoto>> pagerJsonResult = doList(getService(), vo.getPager(), vo.getEntity());
        List<YjfkPhoto> list = pagerJsonResult.getObj().getRecords();
    ...
    @ResponseBody
    public JsonResult<Pager<YjfkPhoto>> list(@ModelAttribute YjfkPhoto entity,@RequestBody PageVo1 pager) {
    ...
    @ResponseBody
    public JsonResult<Pager<YjfkPhoto>> list(@RequestBody BaseVo<YjfkPhoto> vo) {
        return doList(getService(), vo.getPager(), vo.getEntity());
    }

    @ApiOperation(value = "导出", notes="根据vo指定条件进行导出")
    @PostMapping(value = "/export")
    public void export(YjfkPhoto entity, ExportVo vo, HttpServletResponse response) {
        List<YjfkPhoto> datalist;
        if(vo.getArea() == 2){
    ...
    private YjfkPhotoService getService() {
        return this.yjfkPhotoService;
    }
