package com.jnetdata.msp.tlujy.vote_user.service.impl;
 ...
 */
@Service
public class VoteUserServiceImpl extends BaseServiceImpl<VoteUserMapper, VoteUser> implements VoteUserService {


    private String andOr = "and";
    @Override
    public PropertyWrapper<VoteUser> makeSearchWrapper(Map<String, Object> condition) {
        if(condition.get("andOr")!=null) {
            this.andOr = condition.get("andOr").toString();
        }
        PropertyWrapper<VoteUser> pw = createWrapperUtil(condition)
                                        .eq("id")
                                                            .eq("docchannelid")
                                                            .eq("docstatus")
                                                            .eq("singletempkate")
    ...
    @Override
    public VoteUser getEntityAndJoinsById(Long id){
        VoteUser entity = getById(id);
        getJoin(entity, null);
        return entity;
    }

    public void getJoin(VoteUser entity, VoteUser vo) {
        if(vo == null) {
            vo = new VoteUser();
        }
    }
    ...
    @Override
    public void getListJoin(List<VoteUser> list, VoteUser vo) {
        for (VoteUser entity : list) {
            getJoin(entity, vo);
        }
    }
