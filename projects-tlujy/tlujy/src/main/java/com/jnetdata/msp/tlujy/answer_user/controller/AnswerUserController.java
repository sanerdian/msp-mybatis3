package com.jnetdata.msp.tlujy.answer_user.controller;
 ...
 */
@Controller
@RequestMapping("/tlujy/answeruser")
public class AnswerUserController extends BaseController<Long,AnswerUser> {

    final private AnswerUserService answerUserService;
    ...
    public JsonResult<EntityId<Long>> add(@Validated @RequestBody AnswerUser entity) {
    ...
    @PutMapping("/{id}")
    @ResponseBody
    public JsonResult<Void> updateById(@PathVariable("id") Long id, @RequestBody AnswerUser entity) {
    ...
    public JsonResult<Void> updateBatchById(@RequestBody Long[] ids, Integer docstatus) {
        ...
        for (Long id : ids) {
        AnswerUser entity = new AnswerUser();
        entity.setId(id);
    ...
    @ResponseBody
    public JsonResult<Void> doUpdateAllColumnById(@PathVariable("id") Long id, @RequestBody AnswerUser entity) {
    ...
    @GetMapping("/{id}")
    @ResponseBody
    public JsonResult<AnswerUser> getById(@PathVariable("id") Long id) {
    ...
    @GetMapping("/{id}/join")
    @ResponseBody
    public JsonResult<AnswerUser> getjoin(@PathVariable("id") Long id) {
        AnswerUser entity = getService().getEntityAndJoinsById(id);
    ...
    @ResponseBody
    public JsonResult<Pager<AnswerUser>> listjoin(@RequestBody BaseVo<AnswerUser> vo) {
        JsonResult<Pager<AnswerUser>> pagerJsonResult = doList(getService(), vo.getPager(), vo.getEntity());
        List<AnswerUser> list = pagerJsonResult.getObj().getRecords();
    ...
    @ResponseBody
    public JsonResult<Pager<AnswerUser>> list(@ModelAttribute AnswerUser entity,@RequestBody PageVo1 pager) {
    ...
    @ResponseBody
    public JsonResult<Pager<AnswerUser>> list(@RequestBody BaseVo<AnswerUser> vo) {
        return doList(getService(), vo.getPager(), vo.getEntity());
    }

    @ApiOperation(value = "导出", notes="根据vo指定条件进行导出")
    @PostMapping(value = "/export")
    public void export(AnswerUser entity, ExportVo vo, HttpServletResponse response) {
        List<AnswerUser> datalist;
        if(vo.getArea() == 2){
    ...
    private AnswerUserService getService() {
        return this.answerUserService;
    }
