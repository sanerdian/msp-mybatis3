package com.jnetdata.msp.tlujy.investigate_user.controller;
 ...
 */
@Controller
@RequestMapping("/tlujy/investigateuser")
public class InvestigateUserController extends BaseController<Long,InvestigateUser> {

    final private InvestigateUserService investigateUserService;
    ...
    @Autowired
    public InvestigateUserController(InvestigateUserService investigateUserService) {
        this.investigateUserService = investigateUserService;
    }
    ...
    public JsonResult<EntityId<Long>> add(@Validated @RequestBody InvestigateUser entity) {
    ...
    @PutMapping("/{id}")
    @ResponseBody
    public JsonResult<Void> updateById(@PathVariable("id") Long id, @RequestBody InvestigateUser entity) {
    ...
    public JsonResult<Void> updateBatchById(@RequestBody Long[] ids, Integer docstatus) {
        List<InvestigateUser> list = new ArrayList<>();
        for (Long id : ids) {
        InvestigateUser entity = new InvestigateUser();
        entity.setId(id);
    ...
    @ResponseBody
    public JsonResult<Void> doUpdateAllColumnById(@PathVariable("id") Long id, @RequestBody InvestigateUser entity) {
    ...
    @GetMapping("/{id}")
    @ResponseBody
    public JsonResult<InvestigateUser> getById(@PathVariable("id") Long id) {
    ...
    @GetMapping("/{id}/join")
    @ResponseBody
    public JsonResult<InvestigateUser> getjoin(@PathVariable("id") Long id) {
        InvestigateUser entity = getService().getEntityAndJoinsById(id);
    ...
    @ResponseBody
    public JsonResult<Pager<InvestigateUser>> listjoin(@RequestBody BaseVo<InvestigateUser> vo) {
        JsonResult<Pager<InvestigateUser>> pagerJsonResult = doList(getService(), vo.getPager(), vo.getEntity());
        List<InvestigateUser> list = pagerJsonResult.getObj().getRecords();
    ...
    @ResponseBody
    public JsonResult<Pager<InvestigateUser>> list(@ModelAttribute InvestigateUser entity,@RequestBody PageVo1 pager) {
    ...
    @ResponseBody
    public JsonResult<Pager<InvestigateUser>> list(@RequestBody BaseVo<InvestigateUser> vo) {
        return doList(getService(), vo.getPager(), vo.getEntity());
    }

    @ApiOperation(value = "导出", notes="根据vo指定条件进行导出")
    @PostMapping(value = "/export")
    public void export(InvestigateUser entity, ExportVo vo, HttpServletResponse response) {
        List<InvestigateUser> datalist;
        if(vo.getArea() == 2){
    ...
    private InvestigateUserService getService() {
        return this.investigateUserService;
    }
