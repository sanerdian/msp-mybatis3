package com.jnetdata.msp.tlujy.joinuser.service.impl;
 ...
 */
@Service
public class JoinuserServiceImpl extends BaseServiceImpl<JoinuserMapper, Joinuser> implements JoinuserService {


    private String andOr = "and";
    @Override
    public PropertyWrapper<Joinuser> makeSearchWrapper(Map<String, Object> condition) {
        if(condition.get("andOr")!=null) {
            this.andOr = condition.get("andOr").toString();
        }
        PropertyWrapper<Joinuser> pw = createWrapperUtil(condition)
                                        .eq("id")
                                                            .eq("docchannelid")
                                                            .eq("docstatus")
                                                            .eq("singletempkate")
    ...
    @Override
    public Joinuser getEntityAndJoinsById(Long id){
        Joinuser entity = getById(id);
        getJoin(entity, null);
        return entity;
    }

    public void getJoin(Joinuser entity, Joinuser vo) {
        if(vo == null) {
            vo = new Joinuser();
        }
    }
    ...
    @Override
    public void getListJoin(List<Joinuser> list, Joinuser vo) {
        for (Joinuser entity : list) {
            getJoin(entity, vo);
        }
    }
