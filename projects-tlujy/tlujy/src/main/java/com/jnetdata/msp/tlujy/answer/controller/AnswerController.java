package com.jnetdata.msp.tlujy.answer.controller;
 ...
 */
@Controller
@RequestMapping("/tlujy/answer")
public class AnswerController extends BaseController<Long,Answer> {
    ...
    public JsonResult<EntityId<Long>> add(@Validated @RequestBody Answer entity) {
    ...
    @PutMapping("/{id}")
    @ResponseBody
    public JsonResult<Void> updateById(@PathVariable("id") Long id, @RequestBody Answer entity) {
    ...
    @ResponseBody
    public JsonResult<Void> doUpdateAllColumnById(@PathVariable("id") Long id, @RequestBody Answer entity) {
    ...
    @GetMapping("/{id}")
    @ResponseBody
    public JsonResult<Answer> getById(@PathVariable("id") Long id) {
    ...
    @GetMapping("/{id}/join")
    @ResponseBody
    public JsonResult<Answer> getjoin(@PathVariable("id") Long id) {
        Answer entity = getService().getEntityAndJoinsById(id);
    ...
    @ResponseBody
    public JsonResult<Pager<Answer>> listjoin(@RequestBody BaseVo<Answer> vo) {
        JsonResult<Pager<Answer>> pagerJsonResult = doList(getService(), vo.getPager(), vo.getEntity());
        List<Answer> list = pagerJsonResult.getObj().getRecords();
    ...
    @ResponseBody
    public JsonResult<Pager<Answer>> list(@ModelAttribute Answer entity,@RequestBody PageVo1 pager) {
    ...
    @ResponseBody
    public JsonResult<Pager<Answer>> list(@RequestBody BaseVo<Answer> vo) {
        return doList(getService(), vo.getPager(), vo.getEntity());
    }

    @ApiOperation(value = "导出", notes="根据vo指定条件进行导出")
    @PostMapping(value = "/export")
    public void export(Answer entity, ExportVo vo, HttpServletResponse response) {
    ...
    public JsonResult importData(String columnid, MultipartFile file) throws Exception {
        ...
        List<Fieldinfo> list = fieldinfoUtilService.list(new PropertyWrapper<>(Fieldinfo.class).eq("tablename", "JMETA_ANSWER"));
        ...
        list1.forEach(item -> {
            Answer obj = MapUtil.map2Obj(item, Answer.class);
    ...
    private AnswerService getService() {
