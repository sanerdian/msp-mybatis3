package com.jnetdata.msp.tlujy.topic.controller;
 ...
 */
@Controller
@RequestMapping("/tlujy/topic")
public class TopicController extends BaseController<Long,Topic> {
    ...
    public JsonResult<EntityId<Long>> add(@Validated @RequestBody Topic entity) {
    ...
    @PutMapping("/{id}")
    @ResponseBody
    public JsonResult<Void> updateById(@PathVariable("id") Long id, @RequestBody Topic entity) {
    ...
    @ResponseBody
    public JsonResult<Void> doUpdateAllColumnById(@PathVariable("id") Long id, @RequestBody Topic entity) {
    ...
    @GetMapping("/{id}")
    @ResponseBody
    public JsonResult<Topic> getById(@PathVariable("id") Long id) {
    ...
    @GetMapping("/{id}/join")
    @ResponseBody
    public JsonResult<Topic> getjoin(@PathVariable("id") Long id) {
        Topic entity = getService().getEntityAndJoinsById(id);
    ...
    @ResponseBody
    public JsonResult<Pager<Topic>> listjoin(@RequestBody BaseVo<Topic> vo) {
        JsonResult<Pager<Topic>> pagerJsonResult = doList(getService(), vo.getPager(), vo.getEntity());
        List<Topic> list = pagerJsonResult.getObj().getRecords();
    ...
    @ResponseBody
    public JsonResult<Pager<Topic>> list(@ModelAttribute Topic entity,@RequestBody PageVo1 pager) {
    ...
    @ResponseBody
    public JsonResult<Pager<Topic>> list(@RequestBody BaseVo<Topic> vo) {
        return doList(getService(), vo.getPager(), vo.getEntity());
    }

    @ApiOperation(value = "导出", notes="根据vo指定条件进行导出")
    @PostMapping(value = "/export")
    public void export(Topic entity, ExportVo vo, HttpServletResponse response) {
        List<Topic> datalist;
        if(vo.getArea() == 2){
    ...
    public JsonResult importData(String columnid, MultipartFile file) throws Exception {
        ...
        List<Fieldinfo> list = fieldinfoUtilService.list(new PropertyWrapper<>(Fieldinfo.class).eq("tablename", "JMETA_TOPIC"));
        ...
        list1.forEach(item -> {
            Topic obj = MapUtil.map2Obj(item, Topic.class);
