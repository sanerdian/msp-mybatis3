package com.jnetdata.msp.tlujy.integral.controller;
 ...
 */
@Controller
@RequestMapping("/tlujy/integral")
public class IntegralController extends BaseController<Long,Integral> {

    final private IntegralService integralService;
    ...
    public JsonResult<EntityId<Long>> add(@Validated @RequestBody Integral entity) {
    ...
    @PutMapping("/{id}")
    @ResponseBody
    public JsonResult<Void> updateById(@PathVariable("id") Long id, @RequestBody Integral entity) {
    ...
    @ResponseBody
    public JsonResult<Void> doUpdateAllColumnById(@PathVariable("id") Long id, @RequestBody Integral entity) {
    ...
    @GetMapping("/{id}")
    @ResponseBody
    public JsonResult<Integral> getById(@PathVariable("id") Long id) {
    ...
    @GetMapping("/{id}/join")
    @ResponseBody
    public JsonResult<Integral> getjoin(@PathVariable("id") Long id) {
        Integral entity = getService().getEntityAndJoinsById(id);
    ...
    @ResponseBody
    public JsonResult<Pager<Integral>> listjoin(@RequestBody BaseVo<Integral> vo) {
        JsonResult<Pager<Integral>> pagerJsonResult = doList(getService(), vo.getPager(), vo.getEntity());
        List<Integral> list = pagerJsonResult.getObj().getRecords();
    ...
    @ResponseBody
    public JsonResult<Pager<Integral>> list(@ModelAttribute Integral entity,@RequestBody PageVo1 pager) {
    ...
    @ResponseBody
    public JsonResult<Pager<Integral>> list(@RequestBody BaseVo<Integral> vo) {
        return doList(getService(), vo.getPager(), vo.getEntity());
    }

    @ApiOperation(value = "导出", notes="根据vo指定条件进行导出")
    @PostMapping(value = "/export")
    public void export(Integral entity, ExportVo vo, HttpServletResponse response) {
        List<Integral> datalist;
        if(vo.getArea() == 2){
    ...
    private IntegralService getService() {
