package com.jnetdata.msp.tlujy.vote_content.controller;
 ...
 */
@Controller
@RequestMapping("/tlujy/votecontent")
public class VoteContentController extends BaseController<Long,VoteContent> {

    final private VoteContentService voteContentService;
    ...
    public JsonResult<EntityId<Long>> add(@Validated @RequestBody VoteContent entity) {
    ...
    @PutMapping("/{id}")
    @ResponseBody
    public JsonResult<Void> updateById(@PathVariable("id") Long id, @RequestBody VoteContent entity) {
    ...
    public JsonResult<Void> updateBatchById(@RequestBody Long[] ids, Integer docstatus) {
        List<VoteContent> list = new ArrayList<>();
        for (Long id : ids) {
        VoteContent entity = new VoteContent();
        entity.setId(id);
    ...
    @ResponseBody
    public JsonResult<Void> doUpdateAllColumnById(@PathVariable("id") Long id, @RequestBody VoteContent entity) {
    ...
    @GetMapping("/{id}")
    @ResponseBody
    public JsonResult<VoteContent> getById(@PathVariable("id") Long id) {
    ...
    @GetMapping("/{id}/join")
    @ResponseBody
    public JsonResult<VoteContent> getjoin(@PathVariable("id") Long id) {
        VoteContent entity = getService().getEntityAndJoinsById(id);
    ...
    @ResponseBody
    public JsonResult<Pager<VoteContent>> listjoin(@RequestBody BaseVo<VoteContent> vo) {
        JsonResult<Pager<VoteContent>> pagerJsonResult = doList(getService(), vo.getPager(), vo.getEntity());
        List<VoteContent> list = pagerJsonResult.getObj().getRecords();
    ...
    @ResponseBody
    public JsonResult<Pager<VoteContent>> list(@ModelAttribute VoteContent entity,@RequestBody PageVo1 pager) {
    ...
    @ResponseBody
    public JsonResult<Pager<VoteContent>> list(@RequestBody BaseVo<VoteContent> vo) {
        return doList(getService(), vo.getPager(), vo.getEntity());
    }

    @ApiOperation(value = "导出", notes="根据vo指定条件进行导出")
    @PostMapping(value = "/export")
    public void export(VoteContent entity, ExportVo vo, HttpServletResponse response) {
        List<VoteContent> datalist;
        if(vo.getArea() == 2){
    ...
    private VoteContentService getService() {
        return this.voteContentService;
    }
