package com.jnetdata.msp.tlujy.vote_content.service.impl;
 ...
 */
@Service
public class VoteContentServiceImpl extends BaseServiceImpl<VoteContentMapper, VoteContent> implements VoteContentService {


    private String andOr = "and";
    @Override
    public PropertyWrapper<VoteContent> makeSearchWrapper(Map<String, Object> condition) {
        if(condition.get("andOr")!=null) {
            this.andOr = condition.get("andOr").toString();
        }
        PropertyWrapper<VoteContent> pw = createWrapperUtil(condition)
                                        .eq("id")
                                                            .eq("docchannelid")
                                                            .eq("docstatus")
                                                            .eq("singletempkate")
    ...
    @Override
    public VoteContent getEntityAndJoinsById(Long id){
        VoteContent entity = getById(id);
        getJoin(entity, null);
        return entity;
    }

    public void getJoin(VoteContent entity, VoteContent vo) {
        if(vo == null) {
            vo = new VoteContent();
        }
    }
    ...
    @Override
    public void getListJoin(List<VoteContent> list, VoteContent vo) {
        for (VoteContent entity : list) {
            getJoin(entity, vo);
        }
    }
