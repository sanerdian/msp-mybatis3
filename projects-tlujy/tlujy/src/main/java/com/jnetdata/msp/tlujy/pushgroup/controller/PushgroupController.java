package com.jnetdata.msp.tlujy.pushgroup.controller;
 ...
 */
@Controller
@RequestMapping("/tlujy/pushgroup")
public class PushgroupController extends BaseController<Long,Pushgroup> {

    final private PushgroupService pushgroupService;
    ...
    @Autowired
    public PushgroupController(PushgroupService pushgroupService) {
    ...
    public JsonResult<EntityId<Long>> add(@Validated @RequestBody Pushgroup entity) {
    ...
    @PutMapping("/{id}")
    @ResponseBody
    public JsonResult<Void> updateById(@PathVariable("id") Long id, @RequestBody Pushgroup entity) {
    ...
    public JsonResult<Void> updateBatchById(@RequestBody Long[] ids, Integer docstatus) {
        ...
        for (Long id : ids) {
        Pushgroup entity = new Pushgroup();
        entity.setId(id);
    ...
    @ResponseBody
    public JsonResult<Void> doUpdateAllColumnById(@PathVariable("id") Long id, @RequestBody Pushgroup entity) {
    ...
    @GetMapping("/{id}")
    @ResponseBody
    public JsonResult<Pushgroup> getById(@PathVariable("id") Long id) {
    ...
    @GetMapping("/{id}/join")
    @ResponseBody
    public JsonResult<Pushgroup> getjoin(@PathVariable("id") Long id) {
        Pushgroup entity = getService().getEntityAndJoinsById(id);
    ...
    @ResponseBody
    public JsonResult<Pager<Pushgroup>> listjoin(@RequestBody BaseVo<Pushgroup> vo) {
        JsonResult<Pager<Pushgroup>> pagerJsonResult = doList(getService(), vo.getPager(), vo.getEntity());
        List<Pushgroup> list = pagerJsonResult.getObj().getRecords();
    ...
    @ResponseBody
    public JsonResult<Pager<Pushgroup>> list(@ModelAttribute Pushgroup entity,@RequestBody PageVo1 pager) {
    ...
    @ResponseBody
    public JsonResult<Pager<Pushgroup>> list(@RequestBody BaseVo<Pushgroup> vo) {
        return doList(getService(), vo.getPager(), vo.getEntity());
    }

    @ApiOperation(value = "导出", notes="根据vo指定条件进行导出")
    @PostMapping(value = "/export")
    public void export(Pushgroup entity, ExportVo vo, HttpServletResponse response) {
        List<Pushgroup> datalist;
        if(vo.getArea() == 2){
    ...
    private PushgroupService getService() {
        return this.pushgroupService;
    }
