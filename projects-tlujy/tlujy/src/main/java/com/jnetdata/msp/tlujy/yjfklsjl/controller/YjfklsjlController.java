package com.jnetdata.msp.tlujy.yjfklsjl.controller;
 ...
 */
@Controller
@RequestMapping("/tlujy/yjfklsjl")
public class YjfklsjlController extends BaseController<Long,Yjfklsjl> {

    final private YjfklsjlService yjfklsjlService;
    ...
    @Autowired
    public YjfklsjlController(YjfklsjlService yjfklsjlService) {
        this.yjfklsjlService = yjfklsjlService;
    }
    ...
    public JsonResult<EntityId<Long>> add(@Validated @RequestBody Yjfklsjl entity) {
    ...
    @PutMapping("/{id}")
    @ResponseBody
    public JsonResult<Void> updateById(@PathVariable("id") Long id, @RequestBody Yjfklsjl entity) {
    ...
    public JsonResult<Void> updateBatchById(@RequestBody Long[] ids, Integer docstatus) {
        List<Yjfklsjl> list = new ArrayList<>();
        for (Long id : ids) {
        Yjfklsjl entity = new Yjfklsjl();
        entity.setId(id);
    ...
    @ResponseBody
    public JsonResult<Void> doUpdateAllColumnById(@PathVariable("id") Long id, @RequestBody Yjfklsjl entity) {
    ...
    @GetMapping("/{id}")
    @ResponseBody
    public JsonResult<Yjfklsjl> getById(@PathVariable("id") Long id) {
        Yjfklsjl entity = getService().getEntityAndJoinsById(id);
    ...
    @ResponseBody
    public JsonResult<Pager<Yjfklsjl>> list(@ModelAttribute Yjfklsjl entity,@RequestBody PageVo1 pager) {
    ...
    @ResponseBody
    public JsonResult<Pager<Yjfklsjl>> list(@RequestBody BaseVo<Yjfklsjl> vo) {
        JsonResult<Pager<Yjfklsjl>> pagerJsonResult = doList(getService(), vo.getPager(), vo.getEntity());
        List<Yjfklsjl> list = pagerJsonResult.getObj().getRecords();
        getService().getListJoin(list);
        return pagerJsonResult;
    }

    @ApiOperation(value = "导出")
    @PostMapping(value = "/export")
    public void export(Yjfklsjl entity, ExportVo vo, HttpServletResponse response) {
        List<Yjfklsjl> datalist;
        if(vo.getArea() == 2){
    ...
    private YjfklsjlService getService() {
        return this.yjfklsjlService;
    }
