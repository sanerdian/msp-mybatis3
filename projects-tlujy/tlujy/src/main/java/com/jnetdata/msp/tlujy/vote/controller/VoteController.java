package com.jnetdata.msp.tlujy.vote.controller;
 ...
 */
@Controller
@RequestMapping("/tlujy/vote")
public class VoteController extends BaseController<Long,Vote> {
    ...
    public JsonResult<EntityId<Long>> add(@Validated @RequestBody Vote entity) {
    ...
    @PutMapping("/{id}")
    @ResponseBody
    public JsonResult<Void> updateById(@PathVariable("id") Long id, @RequestBody Vote entity) {
    ...
    @ResponseBody
    public JsonResult<Void> doUpdateAllColumnById(@PathVariable("id") Long id, @RequestBody Vote entity) {
    ...
    @GetMapping("/{id}")
    @ResponseBody
    public JsonResult<Vote> getById(@PathVariable("id") Long id) {
    ...
    @GetMapping("/{id}/join")
    @ResponseBody
    public JsonResult<Vote> getjoin(@PathVariable("id") Long id) {
        Vote entity = getService().getEntityAndJoinsById(id);
    ...
    @ResponseBody
    public JsonResult<Pager<Vote>> listjoin(@RequestBody BaseVo<Vote> vo) {
        JsonResult<Pager<Vote>> pagerJsonResult = doList(getService(), vo.getPager(), vo.getEntity());
        List<Vote> list = pagerJsonResult.getObj().getRecords();
    ...
    @ResponseBody
    public JsonResult<Pager<Vote>> list(@ModelAttribute Vote entity,@RequestBody PageVo1 pager) {
    ...
    @ResponseBody
    public JsonResult<Pager<Vote>> list(@RequestBody BaseVo<Vote> vo) {
        return doList(getService(), vo.getPager(), vo.getEntity());
    }

    @ApiOperation(value = "导出", notes="根据vo指定条件进行导出")
    @PostMapping(value = "/export")
    public void export(Vote entity, ExportVo vo, HttpServletResponse response) {
        List<Vote> datalist;
        if(vo.getArea() == 2){
    ...
    public JsonResult importData(String columnid, MultipartFile file) throws Exception {
        ...
        List<Fieldinfo> list = fieldinfoUtilService.list(new PropertyWrapper<>(Fieldinfo.class).eq("tablename", "JMETA_VOTE"));
        ...
        list1.forEach(item -> {
            Vote obj = MapUtil.map2Obj(item, Vote.class);
    ...
    private VoteService getService() {
