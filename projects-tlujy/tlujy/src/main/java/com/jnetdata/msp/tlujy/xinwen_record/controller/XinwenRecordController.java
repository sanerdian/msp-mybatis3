package com.jnetdata.msp.tlujy.xinwen_record.controller;
 ...
 */
@Controller
@RequestMapping("/tlujy/xinwenrecord")
public class XinwenRecordController extends BaseController<Long,XinwenRecord> {

    final private XinwenRecordService xinwenRecordService;
    ...
    @Autowired
    public XinwenRecordController(XinwenRecordService xinwenRecordService) {
        this.xinwenRecordService = xinwenRecordService;
    }
    ...
    public JsonResult<EntityId<Long>> add(@Validated @RequestBody XinwenRecord entity) {
    ...
    @PutMapping("/{id}")
    @ResponseBody
    public JsonResult<Void> updateById(@PathVariable("id") Long id, @RequestBody XinwenRecord entity) {
    ...
    public JsonResult<Void> updateBatchById(@RequestBody Long[] ids, Integer docstatus) {
        List<XinwenRecord> list = new ArrayList<>();
        for (Long id : ids) {
        XinwenRecord entity = new XinwenRecord();
        entity.setId(id);
    ...
    @ResponseBody
    public JsonResult<Void> doUpdateAllColumnById(@PathVariable("id") Long id, @RequestBody XinwenRecord entity) {
    ...
    @GetMapping("/{id}")
    @ResponseBody
    public JsonResult<XinwenRecord> getById(@PathVariable("id") Long id) {
    ...
    @GetMapping("/{id}/join")
    @ResponseBody
    public JsonResult<XinwenRecord> getjoin(@PathVariable("id") Long id) {
        XinwenRecord entity = getService().getEntityAndJoinsById(id);
    ...
    @ResponseBody
    public JsonResult<Pager<XinwenRecord>> listjoin(@RequestBody BaseVo<XinwenRecord> vo) {
        JsonResult<Pager<XinwenRecord>> pagerJsonResult = doList(getService(), vo.getPager(), vo.getEntity());
        List<XinwenRecord> list = pagerJsonResult.getObj().getRecords();
    ...
    @ResponseBody
    public JsonResult<Pager<XinwenRecord>> list(@ModelAttribute XinwenRecord entity,@RequestBody PageVo1 pager) {
    ...
    @ResponseBody
    public JsonResult<Pager<XinwenRecord>> list(@RequestBody BaseVo<XinwenRecord> vo) {
        return doList(getService(), vo.getPager(), vo.getEntity());
    }

    @ApiOperation(value = "导出", notes="根据vo指定条件进行导出")
    @PostMapping(value = "/export")
    public void export(XinwenRecord entity, ExportVo vo, HttpServletResponse response) {
        List<XinwenRecord> datalist;
        if(vo.getArea() == 2){
    ...
    public JsonResult importData(String columnid, MultipartFile file) throws Exception {
        ...
        List<Fieldinfo> list = fieldinfoUtilService.list(new PropertyWrapper<>(Fieldinfo.class).eq("tablename", "JMETA_XINWEN_RECORD"));
        ...
        List<XinwenRecord> list2 = new ArrayList<>();
        list1.forEach(item -> {
            XinwenRecord obj = MapUtil.map2Obj(item, XinwenRecord.class);
    ...
    private XinwenRecordService getService() {
        return this.xinwenRecordService;
    }
