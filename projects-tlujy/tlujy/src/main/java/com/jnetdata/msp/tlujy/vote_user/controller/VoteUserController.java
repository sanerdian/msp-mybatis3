package com.jnetdata.msp.tlujy.vote_user.controller;
 ...
 */
@Controller
@RequestMapping("/tlujy/voteuser")
public class VoteUserController extends BaseController<Long,VoteUser> {

    final private VoteUserService voteUserService;
    ...
    public JsonResult<EntityId<Long>> add(@Validated @RequestBody VoteUser entity) {
    ...
    @PutMapping("/{id}")
    @ResponseBody
    public JsonResult<Void> updateById(@PathVariable("id") Long id, @RequestBody VoteUser entity) {
    ...
    public JsonResult<Void> updateBatchById(@RequestBody Long[] ids, Integer docstatus) {
        List<VoteUser> list = new ArrayList<>();
        for (Long id : ids) {
        VoteUser entity = new VoteUser();
        entity.setId(id);
    ...
    @ResponseBody
    public JsonResult<Void> doUpdateAllColumnById(@PathVariable("id") Long id, @RequestBody VoteUser entity) {
    ...
    @GetMapping("/{id}")
    @ResponseBody
    public JsonResult<VoteUser> getById(@PathVariable("id") Long id) {
    ...
    @GetMapping("/{id}/join")
    @ResponseBody
    public JsonResult<VoteUser> getjoin(@PathVariable("id") Long id) {
        VoteUser entity = getService().getEntityAndJoinsById(id);
    ...
    @ResponseBody
    public JsonResult<Pager<VoteUser>> listjoin(@RequestBody BaseVo<VoteUser> vo) {
        JsonResult<Pager<VoteUser>> pagerJsonResult = doList(getService(), vo.getPager(), vo.getEntity());
        List<VoteUser> list = pagerJsonResult.getObj().getRecords();
    ...
    @ResponseBody
    public JsonResult<Pager<VoteUser>> list(@ModelAttribute VoteUser entity,@RequestBody PageVo1 pager) {
    ...
    @ResponseBody
    public JsonResult<Pager<VoteUser>> list(@RequestBody BaseVo<VoteUser> vo) {
        return doList(getService(), vo.getPager(), vo.getEntity());
    }

    @ApiOperation(value = "导出", notes="根据vo指定条件进行导出")
    @PostMapping(value = "/export")
    public void export(VoteUser entity, ExportVo vo, HttpServletResponse response) {
        List<VoteUser> datalist;
        if(vo.getArea() == 2){
    ...
    private VoteUserService getService() {
        return this.voteUserService;
    }
