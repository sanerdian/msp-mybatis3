package com.jnetdata.msp.tlujy.investigate.controller;
 ...
 */
@Controller
@RequestMapping("/tlujy/investigate")
public class InvestigateController extends BaseController<Long,Investigate> {

    final private InvestigateService investigateService;
    ...
    public JsonResult<EntityId<Long>> add(@Validated @RequestBody Investigate entity) {
    ...
    @PutMapping("/{id}")
    @ResponseBody
    public JsonResult<Void> updateById(@PathVariable("id") Long id, @RequestBody Investigate entity) {
    ...
    public JsonResult<Void> updateBatchById(@RequestBody Long[] ids, Integer docstatus) {
        ...
        for (Long id : ids) {
        Investigate entity = new Investigate();
        entity.setId(id);
    ...
    @ResponseBody
    public JsonResult<Void> doUpdateAllColumnById(@PathVariable("id") Long id, @RequestBody Investigate entity) {
    ...
    @GetMapping("/{id}")
    @ResponseBody
    public JsonResult<Investigate> getById(@PathVariable("id") Long id) {
    ...
    @GetMapping("/{id}/join")
    @ResponseBody
    public JsonResult<Investigate> getjoin(@PathVariable("id") Long id) {
        Investigate entity = getService().getEntityAndJoinsById(id);
    ...
    @ResponseBody
    public JsonResult<Pager<Investigate>> listjoin(@RequestBody BaseVo<Investigate> vo) {
        JsonResult<Pager<Investigate>> pagerJsonResult = doList(getService(), vo.getPager(), vo.getEntity());
        List<Investigate> list = pagerJsonResult.getObj().getRecords();
    ...
    @ResponseBody
    public JsonResult<Pager<Investigate>> list(@ModelAttribute Investigate entity,@RequestBody PageVo1 pager) {
    ...
    @ResponseBody
    public JsonResult<Pager<Investigate>> list(@RequestBody BaseVo<Investigate> vo) {
        return doList(getService(), vo.getPager(), vo.getEntity());
    }

    @ApiOperation(value = "导出", notes="根据vo指定条件进行导出")
    @PostMapping(value = "/export")
    public void export(Investigate entity, ExportVo vo, HttpServletResponse response) {
        List<Investigate> datalist;
        if(vo.getArea() == 2){
    ...
    public JsonResult importData(String columnid, MultipartFile file) throws Exception {
        ...
        List<Fieldinfo> list = fieldinfoUtilService.list(new PropertyWrapper<>(Fieldinfo.class).eq("tablename", "JMETA_INVESTIGATE"));
        ...
        List<Investigate> list2 = new ArrayList<>();
        list1.forEach(item -> {
            Investigate obj = MapUtil.map2Obj(item, Investigate.class);
    ...
    private InvestigateService getService() {
        return this.investigateService;
    }
