package com.jnetdata.msp.tlujy.fjtp.controller;
...
import com.jnetdata.msp.tlujy.fjtp.service.FjtpService;
 ...
 */
@Controller
@RequestMapping("/tlujy/fjtp")
public class FjtpController extends BaseController<Long,Fjtp> {

    final private FjtpService fjtpService;
    ...
    @Autowired
    public FjtpController(FjtpService fjtpService) {
        this.fjtpService = fjtpService;
    }
    ...
    public JsonResult<EntityId<Long>> add(@Validated @RequestBody Fjtp entity) {
    ...
    @PutMapping("/{id}")
    @ResponseBody
    public JsonResult<Void> updateById(@PathVariable("id") Long id, @RequestBody Fjtp entity) {
    ...
    public JsonResult<Void> updateBatchById(@RequestBody Long[] ids, Integer docstatus) {
        List<Fjtp> list = new ArrayList<>();
        for (Long id : ids) {
        Fjtp entity = new Fjtp();
        entity.setId(id);
    ...
    @ResponseBody
    public JsonResult<Void> doUpdateAllColumnById(@PathVariable("id") Long id, @RequestBody Fjtp entity) {
    ...
    @GetMapping("/{id}")
    @ResponseBody
    public JsonResult<Fjtp> getById(@PathVariable("id") Long id) {
    ...
    @GetMapping("/{id}/join")
    @ResponseBody
    public JsonResult<Fjtp> getjoin(@PathVariable("id") Long id) {
        Fjtp entity = getService().getEntityAndJoinsById(id);
    ...
    @ResponseBody
    public JsonResult<Pager<Fjtp>> listjoin(@RequestBody BaseVo<Fjtp> vo) {
        JsonResult<Pager<Fjtp>> pagerJsonResult = doList(getService(), vo.getPager(), vo.getEntity());
        List<Fjtp> list = pagerJsonResult.getObj().getRecords();
    ...
    @ResponseBody
    public JsonResult<Pager<Fjtp>> list(@ModelAttribute Fjtp entity,@RequestBody PageVo1 pager) {
    ...
    @ResponseBody
    public JsonResult<Pager<Fjtp>> list(@RequestBody BaseVo<Fjtp> vo) {
        return doList(getService(), vo.getPager(), vo.getEntity());
    }

    @ApiOperation(value = "导出", notes="根据vo指定条件进行导出")
    @PostMapping(value = "/export")
    public void export(Fjtp entity, ExportVo vo, HttpServletResponse response) {
        List<Fjtp> datalist;
        if(vo.getArea() == 2){
    ...
    private FjtpService getService() {
        return this.fjtpService;
    }
