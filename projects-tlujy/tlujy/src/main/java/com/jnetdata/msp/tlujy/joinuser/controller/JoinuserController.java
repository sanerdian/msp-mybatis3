package com.jnetdata.msp.tlujy.joinuser.controller;
...
import com.jnetdata.msp.tlujy.joinuser.service.JoinuserService;
 ...
 */
@Controller
@RequestMapping("/tlujy/joinuser")
public class JoinuserController extends BaseController<Long,Joinuser> {

    final private JoinuserService joinuserService;
    ...
    @Autowired
    public JoinuserController(JoinuserService joinuserService) {
    ...
    public JsonResult<EntityId<Long>> add(@Validated @RequestBody Joinuser entity) {
    ...
    @PutMapping("/{id}")
    @ResponseBody
    public JsonResult<Void> updateById(@PathVariable("id") Long id, @RequestBody Joinuser entity) {
    ...
    @ResponseBody
    public JsonResult<Void> doUpdateAllColumnById(@PathVariable("id") Long id, @RequestBody Joinuser entity) {
    ...
    @GetMapping("/{id}")
    @ResponseBody
    public JsonResult<Joinuser> getById(@PathVariable("id") Long id) {
    ...
    @GetMapping("/{id}/join")
    @ResponseBody
    public JsonResult<Joinuser> getjoin(@PathVariable("id") Long id) {
        Joinuser entity = getService().getEntityAndJoinsById(id);
    ...
    @ResponseBody
    public JsonResult<Pager<Joinuser>> listjoin(@RequestBody BaseVo<Joinuser> vo) {
        JsonResult<Pager<Joinuser>> pagerJsonResult = doList(getService(), vo.getPager(), vo.getEntity());
        List<Joinuser> list = pagerJsonResult.getObj().getRecords();
    ...
    @ResponseBody
    public JsonResult<Pager<Joinuser>> list(@ModelAttribute Joinuser entity,@RequestBody PageVo1 pager) {
    ...
    @ResponseBody
    public JsonResult<Pager<Joinuser>> list(@RequestBody BaseVo<Joinuser> vo) {
        return doList(getService(), vo.getPager(), vo.getEntity());
    }

    @ApiOperation(value = "导出", notes="根据vo指定条件进行导出")
    @PostMapping(value = "/export")
    public void export(Joinuser entity, ExportVo vo, HttpServletResponse response) {
        List<Joinuser> datalist;
        if(vo.getArea() == 2){
    ...
    private JoinuserService getService() {
        return this.joinuserService;
    }
